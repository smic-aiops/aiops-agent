...
# Sync workflow_manager workflows from this repo into an n8n instance via the Public API.
#
# Required (either):
...
#   WORKFLOW_MANAGER_DIR         : base directory for workflow_manager (default: <repo>/apps/workflow_manager)
#   WORKFLOW_CATALOG_MANAGER_DIR : legacy alias of WORKFLOW_MANAGER_DIR
#   WORKFLOW_DIR_SERVICE_REQUEST : workflow dir override (default: <base>/workflows)
#   WORKFLOW_DIR              : deprecated (use WORKFLOW_DIR_AGENT)
#   WORKFLOW_DIR_AGENT        : directory that stores workflows (e.g. apps/workflow_manager/workflow_catalog/workflows)
...
#   OPENAI_MODEL_API_KEY        : OpenAI-compatible API key (plaintext override; do not commit)
...
#   N8N_POST_SYNC_EXPECTED_WORKFLOW_NAMES : comma-separated workflow names to verify after sync (default: built-in set)
#   N8N_POST_SYNC_WEBHOOK_SMOKE_TEST : "true" (default) to run a webhook smoke test after sync
#   N8N_POST_SYNC_WEBHOOK_SMOKE_TEST_URL : optional override URL for the smoke test
...
if [[ -f "${REPO_ROOT}/scripts/lib/setup_log.sh" ]]; then
  ...
  setup_log_start "service_request" "workflow_manager_deploy_workflows"
  setup_log_install_exit_trap
fi

# NOTE: This script supports both DRY_RUN and N8N_DRY_RUN. Resolve early so we
# can skip endpoint/terraform requirements when doing a dry-run.
...
# Backward-compatible env resolution:
# If WORKFLOW_MANAGER_DIR (or its alias) / WORKFLOW_DIR_SERVICE_REQUEST are provided,
# derive WORKFLOW_DIR_AGENT / prompt / policy defaults in the same way as the legacy wrapper did.
CATALOG_MANAGER_DIR_RAW="${WORKFLOW_MANAGER_DIR:-${WORKFLOW_CATALOG_MANAGER_DIR:-}}"
if [[ -n "${CATALOG_MANAGER_DIR_RAW}" ]]; then
  CATALOG_MANAGER_DIR="$(normalize_dir "${CATALOG_MANAGER_DIR_RAW}")"
else
  CATALOG_MANAGER_DIR="${REPO_ROOT}/apps/workflow_manager"
fi
DEFAULT_WORKFLOW_DIR="${CATALOG_MANAGER_DIR}/service_request/workflows"
WORKFLOW_DIR_SERVICE_REQUEST="${WORKFLOW_DIR_SERVICE_REQUEST:-${DEFAULT_WORKFLOW_DIR}}"
WORKFLOW_DIR_SERVICE_REQUEST="$(normalize_dir "${WORKFLOW_DIR_SERVICE_REQUEST}")"
if [[ -z "${WORKFLOW_DIR_AGENT:-}" ]]; then
...
resolve_n8n_api_key_for_realm() {
  ...
  # Fallback: prefer realm-scoped API key from terraform output (n8n_api_keys_by_realm) when available.
  if command -v terraform >/dev/null 2>&1; then
    v="$(
      terraform -chdir="${REPO_ROOT}" output -json n8n_api_keys_by_realm 2>/dev/null \
...
if [[ -z "${N8N_REALM_OVERLAY_DIR_BASE:-}" ]]; then
  ...
  if [[ -n "${prefix_from_tf}" && "${prefix_from_tf}" != "null" ]]; then
    N8N_REALM_OVERLAY_DIR_BASE="vendor/${prefix_from_tf}/apps/workflow_manager/realms"
  else
    # Backward-compatible default (legacy overlay location).
    N8N_REALM_OVERLAY_DIR_BASE="apps/workflow_manager/realms"
  fi
fi

N8N_DB_ALLOW_UNAUTHORIZED_CERTS="${N8N_DB_ALLOW_UNAUTHORIZED_CERTS:-true}"
...
ACTIVATE="${ACTIVATE:-${N8N_ACTIVATE:-}}"
...
WORKFLOW_DIR="${WORKFLOW_DIR:-apps/workflow_manager/workflow_catalog/workflows}"
...
if [[ ! -f "${TFVARS_FILE}" && -f "terraform.apps.tfvars" ]]; then
...
inject_prompts() {
  ...
  python3 - "${file}" "${PROMPT_DIR}" "${PROMPT_LOCK}" "${existing_file}" "${POLICY_DIR}" "${realm_data_dir}" <<'PY' >"${output}"
import json
import os
import re
import sys
...
def pick_realm_override(filename, subdir):
    if not realm_data_dir:
        return None
    if Path(filename).is_absolute() or len(Path(filename).parts) != 1:
        return None
    candidates = []
    if subdir:
        candidates.append(realm_data_dir / subdir / filename)
    candidates.append(realm_data_dir / filename)
...
ensure_aws_credentials() {
  ...
  if [[ -n "${existing_id}" ]]; then
    ...
    if [[ "${API_STATUS}" == 2* ]]; then
      ...
      echo "[n8n] PATCH /credentials not allowed; using existing AWS credential id." >&2
      cred_id="${existing_id}"
    else
...
ensure_zulip_basic_auth_credentials() {
  ...
  if [[ -z "${password}" && -n "${api_key_param}" ]]; then
    if ! command -v aws >/dev/null 2>&1; then
      echo "[n8n] aws CLI is required to fetch ${api_key_param} from SSM." >&2
      exit 1
    fi
...
PY
  ...
  require_var "ZULIP_BASIC_USERNAME" "${username}"
  require_var "ZULIP_BASIC_PASSWORD" "${password}"
  ...
  local existing_id="${ZULIP_BASIC_CREDENTIAL_ID}"
  if [[ -z "${existing_id}" ]]; then
    api_call "GET" "/credentials?limit=200"
    expect_2xx "GET /credentials"
    existing_id="$(jq -r --arg name "${ZULIP_BASIC_CREDENTIAL_NAME}" '(.data // []) | map(select(.name == $name and .type == "httpBasicAuth")) | .[0].id // empty' <<<"${API_BODY}")"
  fi

  local cred_id=""
  if [[ -n "${existing_id}" ]]; then
    api_call "PATCH" "/credentials/${existing_id}" "${payload}"
    expect_2xx "PATCH /credentials (${ZULIP_BASIC_CREDENTIAL_NAME})"
    cred_id="${existing_id}"
  else
    api_call "POST" "/credentials" "${payload}"
...
ensure_openai_credentials() {
  ...
  local existing_id="${OPENAI_CREDENTIAL_ID}"
  if [[ -z "${existing_id}" ]] && is_truthy "${N8N_REST_CREDENTIALS_FALLBACK:-true}"; then
    existing_id="$(rest_find_credential_id "${OPENAI_CREDENTIAL_NAME}" "openAiApi" || true)"
  fi
  ...
  if [[ -n "${existing_id}" ]]; then
    ...
    for v in "${variants[@]}"; do
      ...
      if [[ "${API_STATUS}" == "405" ]]; then
        if is_truthy "${N8N_REST_CREDENTIALS_FALLBACK:-true}" && rest_upsert_credential "${existing_id}" "${payload}" "${OPENAI_CREDENTIAL_NAME}" "openAiApi"; then
          cred_id="${existing_id}"
          patched="true"
          break
        fi
    ...
    if [[ "${patched}" != "true" ]]; then
      echo "[n8n] PATCH /credentials (${OPENAI_CREDENTIAL_NAME}) failed (HTTP ${API_STATUS})" >&2
      echo "${API_BODY}" >&2
      exit 1
    fi
  else
...
run_post_sync_tests() {
  ...
  # The n8n Public API does not provide a reliable "run workflow now" endpoint.
  # Also, some deployments may not expose webhook endpoints (e.g., queue/webhook split).
  # As a best-effort post-sync verification, confirm the expected workflows are present.
  api_call "GET" "/workflows?limit=250"
  if [[ "${API_STATUS}" != 2* ]]; then
    echo "[test] realm=${realm_label} skipped (GET /workflows failed HTTP ${API_STATUS})." >&2
    return
  fi

  local wf_list
  wf_list="$(extract_workflow_list)"

  local expected=()
  if [[ -n "${N8N_POST_SYNC_EXPECTED_WORKFLOW_NAMES:-}" ]]; then
    local -a parts=()
    IFS=',' read -r -a parts <<<"${N8N_POST_SYNC_EXPECTED_WORKFLOW_NAMES}"
    local part
    for part in "${parts[@]}"; do
    ...
    expected=(
      "aiops-workflows-list"
      "aiops-workflows-get"
      "GitLab Service Catalog Sync"
      "Test GitLab Service Catalog Sync"
    )
  fi
  local missing=()
  local name
  ...
  if [[ "${#expected[@]}" -gt 0 ]]; then
    for name in "${expected[@]}"; do
      if ! jq -e --arg name "${name}" 'map(select(.name == $name)) | length == 1' <<<"${wf_list}" >/dev/null 2>&1; then
        missing+=("${name}")
      fi
    done
  fi
  ...
  if [[ "${#expected[@]}" -gt 0 ]]; then
    if [[ "${#missing[@]}" -gt 0 ]]; then
      echo "[test] realm=${realm_label} missing workflows: ${missing[*]}" >&2
      if is_truthy "${N8N_POST_SYNC_TEST_STRICT:-false}"; then
        exit 1
      fi
    else
      echo "[test] realm=${realm_label} workflows present (best-effort)"
    fi
  fi

  # Optional webhook smoke test. If this returns 404, the instance likely doesn't route webhooks.
  if is_truthy "${N8N_POST_SYNC_WEBHOOK_SMOKE_TEST:-true}" && [[ -z "${N8N_WORKFLOWS_TOKEN:-}" ]]; then
    return
  fi
  if ! is_truthy "${N8N_POST_SYNC_WEBHOOK_SMOKE_TEST:-true}"; then
    return
  fi

  local url="${N8N_POST_SYNC_WEBHOOK_SMOKE_TEST_URL:-${base_url%/}/webhook/catalog/workflows/list?limit=1}"
  local status
  status="$(curl -sS -w '%{http_code}' -o /tmp/aiops_workflow_test_resp.json     -H "Authorization: Bearer ${N8N_WORKFLOWS_TOKEN}"     -X GET     "${url}" || true)"

  if [[ "${status}" == 2* ]]; then
    echo "[test] realm=${realm_label} webhook OK: ${url}"
    rm -f /tmp/aiops_workflow_test_resp.json
    return
  fi

  echo "[test] realm=${realm_label} webhook not available (HTTP ${status})." >&2
  cat /tmp/aiops_workflow_test_resp.json >&2 || true
  rm -f /tmp/aiops_workflow_test_resp.json
  if is_truthy "${N8N_POST_SYNC_TEST_STRICT:-false}"; then
    exit 1
  fi
...
sync_workflows_for_realm() {
  ...
  if [[ -n "${REALM_DATA_DIR}" ]]; then
    echo "[n8n] realm=${realm_label} base_url=${N8N_PUBLIC_API_BASE_URL}, dirs=${WORKFLOW_DIRS_STR}, activate=${ACTIVATE}, dry_run=${DRY_RUN}, realm_data_dir=${REALM_DATA_DIR}"
  else
    echo "[n8n] realm=${realm_label} base_url=${N8N_PUBLIC_API_BASE_URL}, dirs=${WORKFLOW_DIRS_STR}, activate=${ACTIVATE}, dry_run=${DRY_RUN}"
  fi

  # In dry-run we support two modes:
  # - "API-aware plan" (when base_url + api_key are available): compute diffs using n8n API.
  # - "local-only plan" (when missing endpoints/tokens): validate JSON and list files to sync.
  if is_truthy "${DRY_RUN}" && [[ -z "${SKIP_API_WHEN_DRY_RUN:-}" ]]; then
    if [[ -z "${N8N_PUBLIC_API_BASE_URL}" || -z "${N8N_API_KEY}" ]]; then
      SKIP_API_WHEN_DRY_RUN="true"
      export SKIP_API_WHEN_DRY_RUN
      echo "[n8n] dry-run: SKIP_API_WHEN_DRY_RUN=true (base_url/api_key missing)"
    fi
  fi
  ...
  if is_truthy "${DRY_RUN}" && is_truthy "${SKIP_API_WHEN_DRY_RUN:-false}"; then
    local wf_name=""
    local file=""
  ...
  for file in "${files[@]}"; do
    ...
    if [ "${wf_count}" = "0" ]; then
      prompt_tmp="$(mktemp)"
      inject_prompts "${file}" "" "${prompt_tmp}" "${REALM_DATA_DIR:-}"
      desired_payload="$(build_payload "${prompt_tmp}")"
      desired_payload="$(ensure_webhook_ids "${wf_name}" "${desired_payload}")"
      rm -f "${prompt_tmp}"
      if is_truthy "${DRY_RUN}"; then
    ...
    prompt_tmp="$(mktemp)"
    inject_prompts "${file}" "${existing_tmp}" "${prompt_tmp}" "${REALM_DATA_DIR:-}"
    desired_payload="$(build_payload "${prompt_tmp}")"
    desired_payload="$(ensure_webhook_ids "${wf_name}" "${desired_payload}")"
    rm -f "${prompt_tmp}" "${existing_tmp}"
    merged_payload="$(jq -c \
      --argjson desired "${desired_payload}" \
      --argjson existing "${existing_workflow}" \
      ...
      | .nodes = (
          ...
          | map(
              ...
              | if $e == null then
                  ...
                  | .id = ($e.id // .id)
                  | .webhookId = (
                      if ((.webhookId // null) != null and (.webhookId|tostring) != "" and (.webhookId|tostring) != "null") then
                        (.webhookId|tostring)
                      elif (($e.webhookId // null) != null and ($e.webhookId|tostring) != "" and ($e.webhookId|tostring) != "null") then
                        ($e.webhookId|tostring)
                      else
                        null
                      end
                    )
                  | if (.webhookId // null) == null then del(.webhookId) else . end
      ...
      | if ($reset_static_data | test("^(1|true|yes|y|on)$"; "i")) then
          ...
          .staticData = (.staticData // $existing.staticData // null)
        end
      | with_entries(select(.value != null))
      ' <<<"{}")"
  ...
  run_post_sync_tests "${realm_label}" "${N8N_PUBLIC_API_BASE_URL}"
