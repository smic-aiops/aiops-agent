...
本書は `apps/workflow_manager/` の要求（What/Why）を定義します。詳細な利用方法・手順・実装は `apps/workflow_manager/README.md` と `apps/workflow_manager/workflow_catalog/docs/shared/`、ワークフロー定義、データ、同期スクリプトを正とします。
...
本セクションは `apps/workflow_manager/workflow_catalog/docs/shared/dq/dq.md` の設計スコープ/主要リスクを、運用上のユースケースへ落とし込んだものです。
