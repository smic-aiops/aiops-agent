# AI行動仕様（AIS）
## apps/workflow_manager/workflow_catalog/docs/shared/cs/ai_behavior_spec.md
...
## 1. 目的と適用範囲
本書は、`apps/workflow_manager` の AI 支援作業における意図した振る舞い、推論の境界、運用上の制約（出力様式を含む）を定義する。

本仕様は、リポジトリ成果物（要求/検証文書、ワークフロー、同期スクリプト、カタログ/ポリシー等）を変更し得る AI 支援作業に適用される。
...
- 構造化入力（要求、ユースケース、DQ シナリオ、API 契約、ワークフロー定義）を解釈する
...
- 本書（CS）: `apps/workflow_manager/workflow_catalog/docs/shared/cs/ai_behavior_spec.md`
...
- DQ（設計適格性確認）（共通ベース）: `apps/workflow_manager/workflow_catalog/docs/shared/dq/dq.md`
