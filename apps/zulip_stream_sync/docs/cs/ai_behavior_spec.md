# AI行動仕様（AIS）
## apps/zulip_stream_sync/docs/cs/ai_behavior_spec.md
...
## 1. 目的と適用範囲
本書は、`apps/zulip_stream_sync` の AI 支援作業における意図した振る舞い、推論の境界、運用上の制約（出力様式を含む）を定義する。
...
- 推測的または根拠のない結論を避ける
- 仕様/要求/ユースケース/検証（DQ/OQ/PQ）の整合性を維持するため、ユースケース（`apps/zulip_stream_sync/docs/app_requirements.md`）と DQ シナリオ（`apps/zulip_stream_sync/docs/dq/dq.md`）を更新する際は、テンプレート（`scripts/itsm/gitlab/templates/*/docs/usecases/`）を参照し、既存と重複しない形で少なくとも 1 件追加する
...
- 本書（CS）: `apps/zulip_stream_sync/docs/cs/ai_behavior_spec.md`
- 要求/ユースケース: `apps/zulip_stream_sync/docs/app_requirements.md`
- DQ（設計適格性確認）: `apps/zulip_stream_sync/docs/dq/dq.md`
- IQ/OQ/PQ: `apps/zulip_stream_sync/docs/iq/`, `apps/zulip_stream_sync/docs/oq/`, `apps/zulip_stream_sync/docs/pq/`
- ワークフロー定義: `apps/zulip_stream_sync/workflows/`
- 同期/検証スクリプト: `apps/zulip_stream_sync/scripts/`
