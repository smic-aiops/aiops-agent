...
usage() {
  cat <<'USAGE'
Usage: apps/itsm_core/gitlab_dora_metrics_sync/scripts/run_oq.sh [options]
...
if ${DRY_RUN}; then
  ...
  echo "[dry-run] webhook: POST ${N8N_BASE_URL%/}/webhook/gitlab/dora/metrics/sync/oq"
  exit 0
fi
...
if [[ -z "${workflow_id}" ]]; then
  echo "Failed to resolve workflow id for GitLab DORA Metrics Sync" >&2
  exit 1
fi
...
webhook_url="${N8N_BASE_URL%/}/webhook/gitlab/dora/metrics/sync/oq"
