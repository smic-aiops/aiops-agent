# GitLab DORA Metrics Sync 要求（Requirements）

本書は `apps/itsm_core/gitlab_dora_metrics_sync/` の要求（What/Why）を定義します。詳細な利用方法・手順・実装は `apps/itsm_core/gitlab_dora_metrics_sync/README.md` と `apps/itsm_core/gitlab_dora_metrics_sync/docs/`、ワークフロー定義、同期スクリプトを正とします。

## 1. 対象

GitLab Deployments / Merge Requests を運用データとして収集・集計し、日次の DORA 指標（DevOps KPI）を S3 に履歴として保存する n8n ワークフロー群。

## 2. 目的

技術管理（DevOps）の状態を定量化して履歴化し、改善・意思決定（傾向把握/ボトルネック分析等）の材料にする。
...
- UC-DORA-01: 既定（日次/前日 UTC）で GitLab Deployments / Merge Requests を集計し、S3 に履歴出力する
- UC-DORA-02: `N8N_METRICS_TARGET_DATE` 等の入力で対象日付を指定し、集計結果を再現する（監査/調査のための再実行）
- UC-DORA-03: 出力物（`metrics.json`/`gitlab_dora_events.jsonl` 等）が JSON として解析可能であることを確認する（OQ での最低限チェック）
- UC-DORA-04: 期間/対象の取り違いを防ぐ（既定ロジックと手動指定の双方で、出力が意図した期間に整合する）
- UC-DORA-05: GitLab/S3 の認証失敗時に、失敗を明確にし証跡（ログ/出力欠落）を残す
...
- GitLab API から Deployments / Merge Requests を取得する
- 期間（例: 前日 UTC）を切って DORA 指標（デプロイ頻度/変更リードタイム/変更失敗率）を集計する
- 集計結果を S3 に出力する（`metrics.json`/`gitlab_dora_events.jsonl`）
