...
- `apps/itsm_core/cloudwatch_event_notify/docs/oq/oq_cloudwatch_event_notify.md`
...
- 環境変数（`apps/itsm_core/cloudwatch_event_notify/README.md` 記載）が設定済みであること
...
1. `apps/itsm_core/cloudwatch_event_notify/scripts/deploy_workflows.sh` でワークフローを同期する（`DRY_RUN=true` で差分確認も可）。
...
### OQ: ユースケース別カバレッジ（cloudwatch_event_notify）（source: `oq_usecase_coverage.md`）
