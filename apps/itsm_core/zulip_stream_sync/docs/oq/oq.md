...
- 環境変数（`apps/itsm_core/zulip_stream_sync/README.md` 記載）は、Zulip API へ実際に作成/アーカイブを行う場合に設定済みであること
...
> 注意: `apps/itsm_core/zulip_stream_sync/scripts/run_oq.sh` は既定で Webhook を実行します。  
...
### OQ: ユースケース別カバレッジ（zulip_stream_sync）（source: `oq_usecase_coverage.md`）

#### 目的

`apps/itsm_core/zulip_stream_sync/docs/app_requirements.md` に列挙したユースケース（SSoT: `apps/itsm_core/bootstrap/data/templates/*-management/docs/usecases/`）について、**OQ としての実施シナリオが存在する**ことを保証する。
...
- SSoT: `apps/itsm_core/bootstrap/data/templates/service-management/docs/usecases/16_service_onboarding.md.tpl`
- 実施:
...
- アプリ: `apps/itsm_core/zulip_stream_sync`
- ワークフロー: `apps/itsm_core/zulip_stream_sync/workflows/zulip_stream_sync_test.json`
...
- `apps/itsm_core/zulip_stream_sync/workflows/zulip_stream_sync.json` が対象 n8n に同期済みであること
...
- `apps/itsm_core/zulip_stream_sync/docs/oq/evidence/evidence_run_YYYY-MM-DD.md` に、該当 응答（センシティブキーはマスク）を添付する
