...
- 推測的または根拠のない結論を避ける
- 仕様/要求/ユースケース/検証（DQ/OQ/PQ）の整合性を維持するため、共通ベース（`apps/itsm_core/zulip_stream_sync/docs/app_requirements.md`, `apps/itsm_core/zulip_stream_sync/docs/dq/dq.md`）を参照しつつ、realm overlay（`vendor/<name_prefix>/apps/itsm_core/zulip_stream_sync/realms/<realm_key>/docs/app_requirements.md`, `vendor/<name_prefix>/apps/itsm_core/zulip_stream_sync/realms/<realm_key>/docs/dq/dq.md`。`name_prefix` は `terraform output -raw name_prefix` を正とする）を更新する際は、テンプレート（`apps/itsm_core/bootstrap/data/templates/*/docs/usecases/`）を参照し、既存と重複しない形で少なくとも 1 件追加する
