...
- n8n が稼働していること
- 環境変数は `apps/itsm_core/zulip_stream_sync/README.md` を正とする
...
```bash
ACTIVATE=true TEST_WEBHOOK=true apps/itsm_core/zulip_stream_sync/scripts/deploy_workflows.sh
```
...
※ 期待結果は `apps/itsm_core/zulip_stream_sync/docs/oq/oq.md`（`OQ-ZSS-001`）を正とする。
