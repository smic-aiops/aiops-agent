...
**内容**
- 本アプリの仕様・運用・検証の入口を README に集約し、詳細は `apps/itsm_core/zulip_stream_sync/docs/oq/` / `apps/itsm_core/zulip_stream_sync/scripts/` を参照する。
...
- バリデーション成果物（最小）:
  - 本 README
  - OQ 文書: `apps/itsm_core/zulip_stream_sync/docs/oq/oq.md` および `apps/itsm_core/zulip_stream_sync/docs/oq/oq_*.md`（整備: `scripts/generate_oq_md.sh`）
  - OQ 実行補助: `apps/itsm_core/zulip_stream_sync/scripts/run_oq.sh`
...
- Webhook
  - n8n の Webhook ベース URL を `https://n8n.example.com/webhook` とした場合:
    ...
    - テスト: `POST /webhook/zulip/streams/sync/test`
      - `apps/itsm_core/zulip_stream_sync/scripts/deploy_workflows.sh` が、同期後に（`TEST_WEBHOOK=true` の場合）呼び出す
...
### ディレクトリ構成
- `apps/itsm_core/zulip_stream_sync/workflows/`: n8n ワークフロー（JSON）
- `apps/itsm_core/zulip_stream_sync/scripts/`: n8n 同期（アップロード）・OQ 実行スクリプト
- `apps/itsm_core/zulip_stream_sync/docs/cs/`: CS（Configuration Specification: 設計・構成定義）
- `apps/itsm_core/zulip_stream_sync/docs/oq/`: OQ（運用適格性確認）
- `apps/itsm_core/zulip_stream_sync/data/default/prompt/system.md`: サブアプリ単位の中心プロンプト（System 相当）
- `apps/itsm_core/zulip_stream_sync/sql/`: 予約（必要に応じて補助 SQL を配置）

### 同期（n8n Public API へ upsert）
```bash
apps/itsm_core/zulip_stream_sync/scripts/deploy_workflows.sh
```
...
- 代表ケースは `apps/itsm_core/zulip_stream_sync/docs/oq/oq.md` と個別 OQ 文書で定義する。
...
**文書/手順（最小）**
- 同期: `apps/itsm_core/zulip_stream_sync/scripts/deploy_workflows.sh`（`DRY_RUN=true` で差分確認）
...
**文書**
- `apps/itsm_core/zulip_stream_sync/docs/oq/oq.md`（`oq_*.md` から生成）
- 個別シナリオ: `apps/itsm_core/zulip_stream_sync/docs/oq/oq_*.md`

**実行**
- `apps/itsm_core/zulip_stream_sync/scripts/run_oq.sh`
  - 既定は実行（Webhook を叩く）。外部への HTTP 実行なしで確認したい場合は `--dry-run` を指定。

補足:
- OQ 実行前に `scripts/generate_oq_md.sh --app apps/itsm_core/zulip_stream_sync` を実行し、`oq.md` の生成領域を最新化する
