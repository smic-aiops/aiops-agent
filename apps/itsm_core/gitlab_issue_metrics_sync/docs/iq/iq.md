...
- CS（設計・構成定義）: `apps/itsm_core/gitlab_issue_metrics_sync/docs/cs/ai_behavior_spec.md`

## 前提

- n8n が稼働しており、n8n Public API が利用可能であること
- 環境変数は `apps/itsm_core/gitlab_issue_metrics_sync/README.md` 記載（および workflow 内 Sticky Note）を正とする
...
```bash
DRY_RUN=true apps/itsm_core/gitlab_issue_metrics_sync/scripts/deploy_workflows.sh
```

### 2. 同期（反映）

```bash
ACTIVATE=true apps/itsm_core/gitlab_issue_metrics_sync/scripts/deploy_workflows.sh
```
...
※ 実行パラメータ（対象日付など）と期待結果は `apps/itsm_core/gitlab_issue_metrics_sync/docs/oq/oq.md` を正とする。
