...
## Process
- ユースケース拡張:
  - `apps/itsm_core/gitlab_issue_metrics_sync/docs/app_requirements.md` を確認（存在しない場合は作成）し、`scripts/itsm/gitlab/templates/*/docs/usecases/` を参照してユースケースを **1つ以上**追加（既存ユースケースと重複しない）
  - 追加ユースケースに対応するシナリオを **1つ以上** `apps/itsm_core/gitlab_issue_metrics_sync/docs/dq/dq.md` へ組み込み（存在しない場合は作成。既存シナリオと重複しない）
- 仕様確認: `apps/itsm_core/gitlab_issue_metrics_sync/docs/dq/dq.md` を確認し、DQ改善点を10件以上列挙
- DQ修正: 指摘を反映して `apps/itsm_core/gitlab_issue_metrics_sync/docs/dq/dq.md` を修正し、修正内容と理由を短く記録
...
- DQ: `apps/itsm_core/gitlab_issue_metrics_sync/docs/dq/dq.md`
