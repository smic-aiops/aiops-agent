...
**内容**
- 本アプリの仕様・運用・検証の入口を README に集約し、詳細は `apps/itsm_core/gitlab_issue_metrics_sync/docs/oq/` / `apps/itsm_core/gitlab_issue_metrics_sync/scripts/` を参照する。
...
- バリデーション成果物（最小）:
  - 本 README
  - OQ 文書: `apps/itsm_core/gitlab_issue_metrics_sync/docs/oq/oq.md` および `apps/itsm_core/gitlab_issue_metrics_sync/docs/oq/oq_*.md`（整備: `scripts/generate_oq_md.sh`）
  - OQ 実行補助: `apps/itsm_core/gitlab_issue_metrics_sync/scripts/run_oq.sh`
...
- スケジュール（既定）
  - `apps/itsm_core/gitlab_issue_metrics_sync/workflows/gitlab_issue_metrics_sync.json` の Cron で日次実行（既定: 01:30）
- Webhook（OQ / テスト）
...
### ディレクトリ構成
- `apps/itsm_core/gitlab_issue_metrics_sync/workflows/`: n8n ワークフロー（JSON）
- `apps/itsm_core/gitlab_issue_metrics_sync/scripts/`: n8n 同期（アップロード）・OQ 実行スクリプト
- `apps/itsm_core/gitlab_issue_metrics_sync/docs/cs/`: CS（Configuration Specification: 設計・構成定義）
- `apps/itsm_core/gitlab_issue_metrics_sync/docs/oq/`: OQ（運用適格性確認）
- `apps/itsm_core/gitlab_issue_metrics_sync/data/default/prompt/system.md`: サブアプリ単位の中心プロンプト（System 相当）
- `apps/itsm_core/gitlab_issue_metrics_sync/sql/`: 予約（必要に応じて補助 SQL を配置）

### ワークフローの環境変数（代表）
ワークフロー内の `Sticky Note: env`（`apps/itsm_core/gitlab_issue_metrics_sync/workflows/gitlab_issue_metrics_sync.json`）を正とする。
...
```bash
apps/itsm_core/gitlab_issue_metrics_sync/scripts/deploy_workflows.sh
```
...
- 代表ケースは `apps/itsm_core/gitlab_issue_metrics_sync/docs/oq/oq.md` と個別 OQ 文書で定義する。
...
**文書/手順（最小）**
- 同期: `apps/itsm_core/gitlab_issue_metrics_sync/scripts/deploy_workflows.sh`（`DRY_RUN=true` で差分確認）
...
**文書**
- `apps/itsm_core/gitlab_issue_metrics_sync/docs/oq/oq.md`（`oq_*.md` から生成）
- 個別シナリオ: `apps/itsm_core/gitlab_issue_metrics_sync/docs/oq/oq_*.md`

**実行**
- `apps/itsm_core/gitlab_issue_metrics_sync/scripts/run_oq.sh`

補足:
- OQ 実行前に `scripts/generate_oq_md.sh --app apps/itsm_core/gitlab_issue_metrics_sync` を実行し、`oq.md` の生成領域を最新化する
