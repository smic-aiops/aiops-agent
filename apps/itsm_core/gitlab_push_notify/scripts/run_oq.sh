...
usage() {
  cat <<'USAGE'
Usage: apps/itsm_core/gitlab_push_notify/scripts/run_oq.sh [options]
...
ssm_get_parameter() {
  ...
  local -a cmd=(aws --region "${AWS_REGION}" ssm get-parameter --name "${name}" --with-decryption --query 'Parameter.Value' --output text)
  if [[ -n "${AWS_PROFILE}" ]]; then
    cmd=(aws --profile "${AWS_PROFILE}" --region "${AWS_REGION}" ssm get-parameter --name "${name}" --with-decryption --query 'Parameter.Value' --output text)
  fi
...
resolve_name_prefix() {
  ...
  v="$(terraform_output name_prefix)"
...
if ${DRY_RUN}; then
  GITLAB_WEBHOOK_SECRET="${GITLAB_WEBHOOK_SECRET:-<dry-run>}"
else
  if [[ -z "${GITLAB_WEBHOOK_SECRET}" ]]; then
    name_prefix="$(resolve_name_prefix)"
    name_prefix="${name_prefix:-${REALM}}"
    ssm_name="/${name_prefix}/n8n/gitlab/webhook_secret/${REALM}"
    GITLAB_WEBHOOK_SECRET="$(ssm_get_parameter "${ssm_name}")"
  fi

  if [[ -z "${GITLAB_WEBHOOK_SECRET}" ]]; then
    GITLAB_WEBHOOK_YAML="${GITLAB_WEBHOOK_SECRETS_YAML:-}"
    if [[ -z "${GITLAB_WEBHOOK_YAML}" ]]; then
      GITLAB_WEBHOOK_YAML="$(terraform_output gitlab_webhook_secrets_yaml)"
    fi
  ...
  if [[ -z "${GITLAB_WEBHOOK_SECRET}" ]]; then
    echo "Failed to resolve GITLAB_WEBHOOK_SECRET (set env or ensure SSM/terraform outputs are available)" >&2
    exit 1
  fi
fi
...
PY
  ...
  echo "[hint] n8n env (apps/itsm_core/gitlab_push_notify/workflows/gitlab_push_notify.json):"
