...
- GitLab webhook セットアップ: `apps/itsm_core/gitlab_push_notify/scripts/setup_gitlab_project_webhook.sh`（任意）
...
- 必須 env（`GITLAB_WEBHOOK_SECRET`, `ZULIP_*`, `GITLAB_PROJECT_*`）は `apps/itsm_core/gitlab_push_notify/README.md` を正とする
...
```bash
DRY_RUN=true apps/itsm_core/gitlab_push_notify/scripts/deploy_workflows.sh
```

### 2. 同期（反映）

```bash
ACTIVATE=true TEST_WEBHOOK=true apps/itsm_core/gitlab_push_notify/scripts/deploy_workflows.sh
```

### 3. GitLab webhook 設置（任意）

```bash
DRY_RUN=true apps/itsm_core/gitlab_push_notify/scripts/setup_gitlab_project_webhook.sh
```
