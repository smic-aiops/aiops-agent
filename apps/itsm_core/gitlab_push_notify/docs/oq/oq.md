...
- 環境変数（`apps/itsm_core/gitlab_push_notify/README.md` 記載）が設定済みであること
...
- アプリ: `apps/itsm_core/gitlab_push_notify`
- スクリプト:
  - `apps/itsm_core/gitlab_push_notify/scripts/deploy_workflows.sh`
  - `apps/itsm_core/gitlab_push_notify/scripts/setup_gitlab_project_webhook.sh`

#### 受け入れ基準

- `deploy_workflows.sh` により `apps/itsm_core/gitlab_push_notify/workflows/` のワークフローが n8n Public API へ同期される
...
- 実行: `DRY_RUN=true apps/itsm_core/gitlab_push_notify/scripts/deploy_workflows.sh`
- 期待:
...
- 実行: `DRY_RUN=true apps/itsm_core/gitlab_push_notify/scripts/setup_gitlab_project_webhook.sh`
- 期待:
...
- アプリ: `apps/itsm_core/gitlab_push_notify`
- ワークフロー: `apps/itsm_core/gitlab_push_notify/workflows/gitlab_push_notify_test.json`
...
`apps/itsm_core/gitlab_push_notify/scripts/deploy_workflows.sh` は、同期後の自己テスト webhook を実行する際に、Zulip 接続情報を terraform output / SSM から解決して `x-aiops-env-*` ヘッダで一時注入できる。
...
### OQ: ユースケース別カバレッジ（gitlab_push_notify）（source: `oq_usecase_coverage.md`）
