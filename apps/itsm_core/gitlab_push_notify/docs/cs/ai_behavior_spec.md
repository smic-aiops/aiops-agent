...
- 推測的または根拠のない結論を避ける
- 仕様/要求/ユースケース/検証（DQ/OQ/PQ）の整合性を維持するため、ユースケース（`apps/itsm_core/gitlab_push_notify/docs/app_requirements.md`）と DQ シナリオ（`apps/itsm_core/gitlab_push_notify/docs/dq/dq.md`）を更新する際は、テンプレート（`scripts/itsm/gitlab/templates/*/docs/usecases/`）を参照し、既存と重複しない形で少なくとも 1 件追加する
...
- DQ（設計適格性確認）: `apps/itsm_core/gitlab_push_notify/docs/dq/dq.md`
