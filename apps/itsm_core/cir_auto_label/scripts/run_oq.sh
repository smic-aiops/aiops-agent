...
usage() {
  cat <<'USAGE'
Usage: apps/itsm_core/cir_auto_label/scripts/run_oq.sh [options]
...
Notes:
  - OQ (smoke) hits the test webhook:
      POST /webhook/gitlab/cir/auto_label/test
USAGE
}
...
if ${DRY_RUN}; then
  echo "[dry-run] webhook(test): POST ${N8N_BASE_URL%/}/webhook/gitlab/cir/auto_label/test"
  exit 0
fi
