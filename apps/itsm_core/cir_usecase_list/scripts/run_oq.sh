...
usage() {
  cat <<'USAGE'
Usage: apps/itsm_core/cir_usecase_list/scripts/run_oq.sh [options]
...
Notes:
  - OQ (smoke) hits the test webhook:
      POST /webhook/itsm/cir/usecases/approved/list/test
  - Main webhook (dry-run mode):
      POST /webhook/itsm/cir/usecases/approved/list  {"dry_run": true}
  - Webhook endpoints are public by default; `N8N_API_KEY` is optional (sent only if available).
USAGE
}
...
if ! ${DRY_RUN}; then
  if [[ -z "${N8N_API_KEY}" ]]; then
    N8N_API_KEY="$(resolve_realm_scoped_env_only "N8N_API_KEY" "${REALM}")"
  fi
fi
...
if ${DRY_RUN}; then
  echo "[dry-run] webhook(test): POST ${N8N_BASE_URL%/}/webhook/itsm/cir/usecases/approved/list/test"
  echo "[dry-run] webhook(main): POST ${N8N_BASE_URL%/}/webhook/itsm/cir/usecases/approved/list"
  exit 0
fi

api_call "cir-usecase-list-test" "${N8N_BASE_URL%/}/webhook/itsm/cir/usecases/approved/list/test" '{"strict": false}'
api_call "cir-usecase-list" "${N8N_BASE_URL%/}/webhook/itsm/cir/usecases/approved/list" '{"dry_run": true}'
