...
本書は `apps/itsm_core/integrations/gitlab_issue_metrics_sync/` の要求（What/Why）を定義します。詳細な利用方法・手順・実装は `apps/itsm_core/integrations/gitlab_issue_metrics_sync/README.md` と `apps/itsm_core/integrations/gitlab_issue_metrics_sync/docs/`、ワークフロー定義、同期スクリプトを正とします。
...
本セクションは `apps/itsm_core/integrations/gitlab_issue_metrics_sync/docs/dq/dq.md` の設計スコープ/主要リスクを、運用上のユースケースへ落とし込んだものです。
