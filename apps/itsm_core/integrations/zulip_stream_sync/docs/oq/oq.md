...
- n8n に次のワークフローが同期済みであること
  - `apps/itsm_core/integrations/zulip_stream_sync/workflows/zulip_stream_sync.json`
  - `apps/itsm_core/integrations/zulip_stream_sync/workflows/zulip_stream_sync_test.json`
- 環境変数（`apps/itsm_core/integrations/zulip_stream_sync/README.md` 記載）は、Zulip API へ実際に作成/アーカイブを行う場合に設定済みであること
...
> 注意: `apps/itsm_core/integrations/zulip_stream_sync/scripts/run_oq.sh` は既定で Webhook を実行します。  
...
- `apps/itsm_core/integrations/zulip_stream_sync/workflows/zulip_stream_sync.json` / `apps/itsm_core/integrations/zulip_stream_sync/workflows/zulip_stream_sync_test.json` が対象 n8n に同期済みであること
...
- `apps/itsm_core/integrations/zulip_stream_sync/docs/oq/evidence/evidence_run_YYYY-MM-DD.md` へ、実施日時・対象 realm・応答（センシティブ情報はマスク済み）を記録する
...
- `apps/itsm_core/integrations/zulip_stream_sync/workflows/zulip_stream_sync.json` が対象 n8n に同期済みであること
...
- `apps/itsm_core/integrations/zulip_stream_sync/docs/oq/evidence/evidence_run_YYYY-MM-DD.md` に、該当 응答（センシティブキーはマスク）を添付する
