...
- n8n に次のワークフローが同期済みであること
  - `apps/itsm_core/integrations/zulip_gitlab_issue_sync/workflows/zulip_gitlab_issue_sync.json`
...
- 同期対象の Zulip stream/topic に投稿が存在すること
  ...
  - ローカル実行でテスト投稿を用意する場合は `apps/itsm_core/integrations/zulip_gitlab_issue_sync/scripts/run_oq.sh --prepare-test-user --prepare-decision` を使用（realm bot で `/oq-seed` と `/decision` を投稿します）
