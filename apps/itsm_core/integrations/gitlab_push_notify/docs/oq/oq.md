...
- 環境変数（`apps/itsm_core/integrations/gitlab_push_notify/README.md` 記載）が設定済みであること
...
- アプリ: `apps/itsm_core/integrations/gitlab_push_notify`
- スクリプト:
  - `apps/itsm_core/integrations/gitlab_push_notify/scripts/deploy_workflows.sh`
  - `apps/itsm_core/integrations/gitlab_push_notify/scripts/setup_gitlab_project_webhook.sh`

#### 受け入れ基準

- `deploy_workflows.sh` により `apps/itsm_core/integrations/gitlab_push_notify/workflows/` のワークフローが n8n Public API へ同期される
...
- 実行: `DRY_RUN=true apps/itsm_core/integrations/gitlab_push_notify/scripts/deploy_workflows.sh`
- 期待:
...
- 実行: `DRY_RUN=true apps/itsm_core/integrations/gitlab_push_notify/scripts/setup_gitlab_project_webhook.sh`
- 期待:
