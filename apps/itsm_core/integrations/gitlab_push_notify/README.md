...
**内容**
- 本アプリの仕様・運用・検証の入口を README に集約し、詳細は `apps/itsm_core/integrations/gitlab_push_notify/docs/oq/` / `apps/itsm_core/integrations/gitlab_push_notify/scripts/` を参照する。
...
- バリデーション成果物（最小）:
  - 本 README
  - OQ 文書: `apps/itsm_core/integrations/gitlab_push_notify/docs/oq/oq.md` および `apps/itsm_core/integrations/gitlab_push_notify/docs/oq/oq_*.md`（整備: `scripts/generate_oq_md.sh`）
  - OQ 実行補助: `apps/itsm_core/integrations/gitlab_push_notify/scripts/run_oq.sh`
...
- Webhook
  - n8n の Webhook ベース URL を `https://n8n.example.com/webhook` とした場合:
    ...
    - テスト: `POST /webhook/gitlab/push/notify/test`
      - `apps/itsm_core/integrations/gitlab_push_notify/scripts/deploy_workflows.sh` が、同期後に（`TEST_WEBHOOK=true` の場合）呼び出す
...
### ディレクトリ構成
- `apps/itsm_core/integrations/gitlab_push_notify/workflows/`: n8n ワークフロー（JSON）
- `apps/itsm_core/integrations/gitlab_push_notify/scripts/`: n8n 同期・GitLab webhook 登録の補助スクリプト
- `apps/itsm_core/integrations/gitlab_push_notify/docs/cs/`: CS（Configuration Specification: 設計・構成定義）
- `apps/itsm_core/integrations/gitlab_push_notify/docs/oq/`: OQ（運用適格性確認）

### セットアップ手順（最短）
0. レルム用の GitLab グループ/プロジェクトを準備: `scripts/itsm/gitlab/itsm_bootstrap_realms.sh`
1. ワークフローを n8n に同期: `apps/itsm_core/integrations/gitlab_push_notify/scripts/deploy_workflows.sh`
2. GitLab のプロジェクト webhook を登録（自動化）: `apps/itsm_core/integrations/gitlab_push_notify/scripts/setup_gitlab_project_webhook.sh`
...
- 代表ケースは `apps/itsm_core/integrations/gitlab_push_notify/docs/oq/oq.md` と個別 OQ 文書で定義する。
...
**文書/手順（最小）**
- 同期: `apps/itsm_core/integrations/gitlab_push_notify/scripts/deploy_workflows.sh`（`DRY_RUN=true` で差分確認）
- （任意）GitLab webhook セットアップ: `apps/itsm_core/integrations/gitlab_push_notify/scripts/setup_gitlab_project_webhook.sh`
...
**文書**
- `apps/itsm_core/integrations/gitlab_push_notify/docs/oq/oq.md`（`oq_*.md` から生成）
- 個別シナリオ: `apps/itsm_core/integrations/gitlab_push_notify/docs/oq/oq_*.md`

**実行**
- `apps/itsm_core/integrations/gitlab_push_notify/scripts/run_oq.sh`

補足:
- OQ 実行前に `scripts/generate_oq_md.sh --app apps/itsm_core/integrations/gitlab_push_notify` を実行し、`oq.md` の生成領域を最新化する
