...
usage() {
  cat <<'USAGE'
Usage:
  apps/itsm_core/integrations/gitlab_backfill_to_sor/scripts/backfill_gitlab_decisions_to_sor.sh [options]

Purpose:
  - Trigger the n8n workflow that scans GitLab Issues/Notes and backfills decisions into ITSM SoR (itsm.audit_event).
  - Decision classification is performed by LLM inside n8n (this script does NOT call the LLM).

Workflow:
  - apps/itsm_core/integrations/gitlab_backfill_to_sor/workflows/gitlab_decision_backfill_to_sor.json
...
Options:
  ...
  --since ISO8601          Optional -> payload.since
  --mode MODE              recall|precision (default: recall) -> payload.mode

  --dry-run                Do not insert into SoR (default)
  --execute                Insert into SoR (payload.dry_run=false)
  --plan-only              No GitLab/LLM calls; return computed plan only (payload.plan_only=true)
...
Environment overrides:
  ...
  REALM_KEY, PROJECT_IDS, SINCE, MODE, DRY_RUN, PLAN_ONLY
USAGE
}
...
echo "[itsm] n8n webhook: ${N8N_URL}/webhook/gitlab/decision/backfill/sor"
...
curl "${curl_flags[@]}" "${N8N_URL}/webhook/gitlab/decision/backfill/sor"
echo
