...
- ワークフロー:
  - `apps/itsm_core/integrations/cloudwatch_event_notify/workflows/cloudwatch_event_notify.json`
  - `apps/itsm_core/integrations/cloudwatch_event_notify/workflows/cloudwatch_event_notify_test.json`
- 同期スクリプト: `apps/itsm_core/integrations/cloudwatch_event_notify/scripts/deploy_workflows.sh`
- CS（設計・構成定義）: `apps/itsm_core/integrations/cloudwatch_event_notify/docs/cs/ai_behavior_spec.md`
- OQ: `apps/itsm_core/integrations/cloudwatch_event_notify/docs/oq/oq_cloudwatch_event_notify.md`（互換: `apps/itsm_core/integrations/cloudwatch_event_notify/docs/oq/oq.md`）
...
```bash
DRY_RUN=true apps/itsm_core/integrations/cloudwatch_event_notify/scripts/deploy_workflows.sh
```

### 2. 同期（反映）

```bash
ACTIVATE=true TEST_WEBHOOK=true apps/itsm_core/integrations/cloudwatch_event_notify/scripts/deploy_workflows.sh
```
...
※ 実行例は OQ に集約（`apps/itsm_core/integrations/cloudwatch_event_notify/docs/oq/oq_cloudwatch_event_notify.md`）。
