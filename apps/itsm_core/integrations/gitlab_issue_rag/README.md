...
**内容**
- 本アプリの仕様・運用・検証の入口を README に集約し、詳細は `apps/itsm_core/integrations/gitlab_issue_rag/docs/oq/` / `apps/itsm_core/integrations/gitlab_issue_rag/scripts/` を参照する。
...
- バリデーション成果物（最小）:
  - 本 README
  - OQ 文書: `apps/itsm_core/integrations/gitlab_issue_rag/docs/oq/oq.md` および `apps/itsm_core/integrations/gitlab_issue_rag/docs/oq/oq_*.md`（整備: `scripts/generate_oq_md.sh`）
  - OQ 実行補助: `apps/itsm_core/integrations/gitlab_issue_rag/scripts/run_oq.sh`
...
### ディレクトリ構成
- `apps/itsm_core/integrations/gitlab_issue_rag/workflows/`: n8n ワークフロー（JSON）
- `apps/itsm_core/integrations/gitlab_issue_rag/scripts/`: n8n 同期（アップロード）・OQ 実行スクリプト
- `apps/itsm_core/integrations/gitlab_issue_rag/sql/`: pgvector テーブル/インデックス定義
- `apps/itsm_core/integrations/gitlab_issue_rag/docs/cs/`: CS（Configuration Specification: 設計・構成定義）
- `apps/itsm_core/integrations/gitlab_issue_rag/docs/oq/`: OQ（運用適格性確認）

### セットアップ手順（最短）
1. pgvector のテーブルを作成（初回のみ）:
   - `psql ... -f apps/itsm_core/integrations/gitlab_issue_rag/sql/gitlab_issue_rag.sql`
2. n8n に Postgres 資格情報を作成（ワークフローの Postgres ノードに紐付くよう設定）
3. ワークフローを n8n に同期:
   - `apps/itsm_core/integrations/gitlab_issue_rag/scripts/deploy_issue_rag_workflows.sh`
...
- `WORKFLOW_DIR`（既定 `apps/itsm_core/integrations/gitlab_issue_rag/workflows`）
...
- 代表ケースは `apps/itsm_core/integrations/gitlab_issue_rag/docs/oq/oq.md` とシナリオ別 OQ 文書で定義する。
...
**文書/手順（最小）**
- 同期: `apps/itsm_core/integrations/gitlab_issue_rag/scripts/deploy_issue_rag_workflows.sh`（`DRY_RUN=true` で差分確認）
- 初期 DB: `apps/itsm_core/integrations/gitlab_issue_rag/sql/gitlab_issue_rag.sql` の適用
...
**文書**
- OQ（概要）: `apps/itsm_core/integrations/gitlab_issue_rag/docs/oq/oq.md`
- シナリオ別: `apps/itsm_core/integrations/gitlab_issue_rag/docs/oq/oq_*.md`

**実行**
- `apps/itsm_core/integrations/gitlab_issue_rag/scripts/run_oq.sh`

補足:
- OQ 実行前に `scripts/generate_oq_md.sh --app apps/itsm_core/integrations/gitlab_issue_rag` を実行し、`oq.md` の生成領域を最新化する
