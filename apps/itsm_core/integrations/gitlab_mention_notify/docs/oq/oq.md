...
- n8n に次のワークフローが同期済みであること
  - `apps/itsm_core/integrations/gitlab_mention_notify/workflows/gitlab_mention_notify.json`
- 環境変数（`apps/itsm_core/integrations/gitlab_mention_notify/README.md` 記載）が設定済みであること
...
- 実行: `DRY_RUN=true apps/itsm_core/integrations/gitlab_mention_notify/scripts/deploy_workflows.sh`
- 期待: 同期対象の差分/予定内容が出力される

##### TC-02: グループ webhook 登録（dry-run）

- 実行: `DRY_RUN=true apps/itsm_core/integrations/gitlab_mention_notify/scripts/setup_gitlab_group_webhook.sh`
- 期待: 登録/更新予定が出力される
...
- スクリプト: `apps/itsm_core/integrations/gitlab_mention_notify/scripts/run_oq.sh`

#### 受け入れ基準

- `GITLAB_MENTION_NOTIFY_DRY_RUN=true` で Zulip 送信を抑止しつつ、抽出/宛先解決/整形の確認ができる
- `apps/itsm_core/integrations/gitlab_mention_notify/scripts/run_oq.sh` により、webhook 疎通（OQ）を再現性のある手順で確認できる
...
- 実行: `apps/itsm_core/integrations/gitlab_mention_notify/scripts/run_oq.sh`
...
- アプリ: `apps/itsm_core/integrations/gitlab_mention_notify`
- ワークフロー: `apps/itsm_core/integrations/gitlab_mention_notify/workflows/gitlab_mention_notify.json`
- 参照: `apps/itsm_core/integrations/gitlab_mention_notify/data/mention_exclude_words.txt`（運用上の除外語）
