...
- GitLab webhook セットアップ（任意）: `apps/itsm_core/integrations/gitlab_mention_notify/scripts/setup_gitlab_group_webhook.sh`
...
- 必須 env（`GITLAB_WEBHOOK_SECRET`, `ZULIP_*`）は `apps/itsm_core/integrations/gitlab_mention_notify/README.md` を正とする
...
```bash
DRY_RUN=true apps/itsm_core/integrations/gitlab_mention_notify/scripts/deploy_workflows.sh
```

### 2. 同期（反映）

```bash
ACTIVATE=true apps/itsm_core/integrations/gitlab_mention_notify/scripts/deploy_workflows.sh
```

### 3. GitLab webhook の設置（任意）

```bash
DRY_RUN=true apps/itsm_core/integrations/gitlab_mention_notify/scripts/setup_gitlab_group_webhook.sh
```
