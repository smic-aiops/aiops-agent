...
**内容**
- 本 README を「運用/検証の入口」とし、詳細は `apps/itsm_core/integrations/gitlab_mention_notify/docs/` 配下（CS/OQ/PQ）へ分離する。
...
- バリデーション成果物（最小）:
  ...
  - CS: `apps/itsm_core/integrations/gitlab_mention_notify/docs/cs/`
  - OQ: `apps/itsm_core/integrations/gitlab_mention_notify/docs/oq/oq.md` および `apps/itsm_core/integrations/gitlab_mention_notify/docs/oq/oq_*.md`（整備: `scripts/generate_oq_md.sh`）
  - PQ: `apps/itsm_core/integrations/gitlab_mention_notify/docs/pq/pq.md`
  - OQ 実行補助: `apps/itsm_core/integrations/gitlab_mention_notify/scripts/run_oq.sh`
...
### ディレクトリ構成
- `apps/itsm_core/integrations/gitlab_mention_notify/workflows/`: n8n ワークフロー（JSON）
- `apps/itsm_core/integrations/gitlab_mention_notify/scripts/`: n8n 同期・GitLab group webhook 登録の補助スクリプト
- `apps/itsm_core/integrations/gitlab_mention_notify/data/`: サンプル（除外語、ユーザーマッピング JSON 等）
- `apps/itsm_core/integrations/gitlab_mention_notify/docs/cs/`: CS（Configuration Specification: 設計・構成定義）
- `apps/itsm_core/integrations/gitlab_mention_notify/docs/oq/`: OQ（運用適格性確認）
- `apps/itsm_core/integrations/gitlab_mention_notify/docs/pq/`: PQ（性能適格性確認）

### セットアップ手順（最短）
1. ワークフローを n8n に同期: `apps/itsm_core/integrations/gitlab_mention_notify/scripts/deploy_workflows.sh`
2. GitLab 側 webhook を登録（自動化・任意）: `apps/itsm_core/integrations/gitlab_mention_notify/scripts/setup_gitlab_group_webhook.sh`
...
`apps/itsm_core/integrations/gitlab_mention_notify/scripts/setup_gitlab_group_webhook.sh` は Terraform の出力を参照して、複数レルムの group webhook を作成/更新する。
...
- 代表ケースは `apps/itsm_core/integrations/gitlab_mention_notify/docs/oq/oq.md` と個別 OQ 文書で定義する。
- 性能面は PQ（`apps/itsm_core/integrations/gitlab_mention_notify/docs/pq/pq.md`）の最小手順で補う。
...
**文書/手順（最小）**
- 同期: `apps/itsm_core/integrations/gitlab_mention_notify/scripts/deploy_workflows.sh`（`DRY_RUN=true` で差分確認）
- （任意）GitLab webhook セットアップ: `apps/itsm_core/integrations/gitlab_mention_notify/scripts/setup_gitlab_group_webhook.sh`
...
**文書**
- `apps/itsm_core/integrations/gitlab_mention_notify/docs/oq/oq.md`（`oq_*.md` から生成）
- 個別シナリオ: `apps/itsm_core/integrations/gitlab_mention_notify/docs/oq/oq_*.md`

**実行**
- `apps/itsm_core/integrations/gitlab_mention_notify/scripts/run_oq.sh`

補足:
- OQ 実行前に `scripts/generate_oq_md.sh --app apps/itsm_core/integrations/gitlab_mention_notify` を実行し、`oq.md` の生成領域を最新化する
...
**文書/手順（最小）**
- `apps/itsm_core/integrations/gitlab_mention_notify/docs/pq/pq.md`
