...
usage() {
  cat <<'USAGE'
Usage:
  apps/itsm_core/aiops_approval_history_backfill_to_sor/scripts/backfill_itsm_sor_from_aiops_approval_history.sh [options]
...
if [ -z "${NAME_PREFIX}" ] && command -v terraform >/dev/null 2>&1; then
  tf_name_prefix="$(terraform -chdir="${REPO_ROOT}" output -raw name_prefix 2>/dev/null || true)"
  tf_name_prefix="$(printf '%s' "${tf_name_prefix}" | tr -d '\r' | head -n 1)"
  case "${tf_name_prefix}" in
    null|"") : ;;
    *)
      if [[ "${tf_name_prefix}" =~ ^[A-Za-z0-9][A-Za-z0-9._-]*$ ]]; then
...
if [ -z "${DB_HOST}" ] || [ -z "${DB_PORT}" ] || [ -z "${DB_NAME}" ] || [ -z "${DB_USER}" ] || [ -z "${DB_PASSWORD}" ]; then
  if [[ "${DRY_RUN}" == "true" ]]; then
    echo "[dry-run] DB connection info is missing. Skipping execution preview."
    echo ""
    echo "Provide DB_HOST/DB_PORT/DB_NAME/DB_USER/DB_PASSWORD, or ensure SSM params are readable:"
    ...
    echo ""
    echo "To execute the backfill, re-run with --execute after resolving DB connectivity."
    exit 0
  fi
