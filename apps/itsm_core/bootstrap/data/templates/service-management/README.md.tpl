...
4. CI の有効化
   ...
   - テンプレ適用: `.gitlab-ci.yml` が無い場合は `apps/itsm_core/bootstrap/data/templates/service-management/.gitlab-ci.yml.tpl` をルートに配置して有効化
...
6. グループCI変数の自動設定（GitLabトークン）
   - `apps/itsm_core/bootstrap/scripts/refresh_realm_group_tokens_with_bot_cleanup.sh` が各レルムのグループアクセストークンを発行（旧トークン削除時に group bot を無効化/削除）
