...
5. CI で CMDB の必須項目を検証し、構成逸脱を防ぐ  
   - `.gitlab-ci.yml` が無い場合は `apps/itsm_core/bootstrap/data/templates/service-management/.gitlab-ci.yml.tpl` をルートに配置して有効化
...
- CI ジョブ: `cmdb:zulip_stream_sync`（テンプレ: `apps/itsm_core/bootstrap/data/templates/service-management/.gitlab-ci.yml.tpl`）
- 同期スクリプト: `scripts/cmdb/sync_zulip_streams.sh`（テンプレ: `apps/itsm_core/bootstrap/data/templates/service-management/scripts/cmdb/sync_zulip_streams.sh`）
