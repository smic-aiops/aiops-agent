...
**内容**
- 本アプリの仕様・運用・検証の入口を README に集約し、詳細は `apps/itsm_core/gitlab_issue_rag/docs/oq/` / `apps/itsm_core/gitlab_issue_rag/scripts/` を参照する。
...
- バリデーション成果物（最小）:
  - 本 README
  - OQ 文書: `apps/itsm_core/gitlab_issue_rag/docs/oq/oq.md` および `apps/itsm_core/gitlab_issue_rag/docs/oq/oq_*.md`（整備: `scripts/generate_oq_md.sh`）
  - OQ 実行補助: `apps/itsm_core/gitlab_issue_rag/scripts/run_oq.sh`
...
- `WORKFLOW_DIR`（既定 `apps/itsm_core/gitlab_issue_rag/workflows`）
...
- 代表ケースは `apps/itsm_core/gitlab_issue_rag/docs/oq/oq.md` とシナリオ別 OQ 文書で定義する。
...
**文書**
- OQ（概要）: `apps/itsm_core/gitlab_issue_rag/docs/oq/oq.md`
- シナリオ別: `apps/itsm_core/gitlab_issue_rag/docs/oq/oq_*.md`

**実行**
- `apps/itsm_core/gitlab_issue_rag/scripts/run_oq.sh`

補足:
- OQ 実行前に `scripts/generate_oq_md.sh --app apps/itsm_core/gitlab_issue_rag` を実行し、`oq.md` の生成領域を最新化する
