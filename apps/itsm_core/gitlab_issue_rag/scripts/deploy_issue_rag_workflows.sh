...
#   WORKFLOW_DIR (default: apps/itsm_core/gitlab_issue_rag/workflows)
#   N8N_REALM_OVERLAY_DIR_BASE : base dir for realm overlay overrides (workflows) (default: vendor/<name_prefix>/apps/itsm_core/gitlab_issue_rag/realms; fallback: apps/itsm_core/gitlab_issue_rag/realms)
...
for realm in "${TARGET_REALMS[@]}"; do
  ...
  echo "[n8n] realm=${realm_label} base_url=${N8N_PUBLIC_API_BASE_URL} files=${#files_for_realm[@]}"
