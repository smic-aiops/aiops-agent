...
- 同期スクリプト: `apps/itsm_core/gitlab_mention_notify/scripts/deploy_workflows.sh`
- GitLab webhook セットアップ（任意）: `apps/itsm_core/gitlab_mention_notify/scripts/setup_gitlab_group_webhook.sh`
- OQ: `apps/itsm_core/gitlab_mention_notify/docs/oq/oq.md`
- CS: `apps/itsm_core/gitlab_mention_notify/docs/cs/ai_behavior_spec.md`

## 前提

- n8n が稼働していること
- GitLab 側（Group Webhook）の設定を変更できる権限があること（任意）
- 必須 env（`GITLAB_WEBHOOK_SECRET`, `ZULIP_*`）は `apps/itsm_core/gitlab_mention_notify/README.md` を正とする
...
```bash
DRY_RUN=true apps/itsm_core/gitlab_mention_notify/scripts/deploy_workflows.sh
```

### 2. 同期（反映）

```bash
ACTIVATE=true apps/itsm_core/gitlab_mention_notify/scripts/deploy_workflows.sh
```

### 3. GitLab webhook の設置（任意）

```bash
DRY_RUN=true apps/itsm_core/gitlab_mention_notify/scripts/setup_gitlab_group_webhook.sh
```
