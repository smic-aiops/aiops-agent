...
usage() {
  cat <<'USAGE'
Usage: apps/itsm_core/itsm_sla_metrics_sync/scripts/run_oq.sh [options]
...
if ${DRY_RUN}; then
  N8N_API_KEY="${N8N_API_KEY:-<unresolved_n8n_api_key>}"
else
  N8N_API_KEY="$(resolve_n8n_api_key_for_realm "${REALM}")"
  if [[ -z "${N8N_API_KEY}" ]]; then
    echo "Failed to resolve N8N API key" >&2
    exit 1
  fi
fi
...
if ${DRY_RUN}; then
  ...
  echo "[dry-run] webhook: POST ${N8N_BASE_URL%/}/webhook/itsm/sla/metrics/sync/oq"
  exit 0
fi
...
if [[ -z "${workflow_id}" ]]; then
  echo "Failed to resolve workflow id for ITSM SLA Metrics Sync" >&2
  exit 1
fi
...
payload="$(python3 - "${effective_target_date}" <<'PY'
import json
import sys
...
webhook_url="${N8N_BASE_URL%/}/webhook/itsm/sla/metrics/sync/oq"
