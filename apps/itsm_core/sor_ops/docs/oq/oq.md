# OQ（運用適格性確認）: SoR Ops（ITSM Core）

## 目的

SoR 運用スクリプト（DDL/RLS/保持/匿名化等）が、dry-run（計画/SQL 出力）で **安全に実行できる** ことを確認する。

## 前提

- 秘匿情報を平文で扱わない（tfvars 直読み禁止）。接続情報は `terraform output` / SSM を正とする。
...
### 一覧
- [oq_sor_ops_dry_run.md](oq_sor_ops_dry_run.md)
- [oq_sor_ops_n8n_smoke.md](oq_sor_ops_n8n_smoke.md)
- [oq_usecase_coverage.md](oq_usecase_coverage.md)

---

### OQ: SoR Ops - dry-run（計画/SQL 出力）（source: `oq_sor_ops_dry_run.md`）
...
### OQ: SoR Ops - n8n スモークテスト（保持/PII redaction）（source: `oq_sor_ops_n8n_smoke.md`）
...
### OQ: ユースケース別カバレッジ（sor_ops）（source: `oq_usecase_coverage.md`）

#### 目的

`apps/itsm_core/sor_ops/docs/app_requirements.md` に列挙したユースケース（SSoT: `apps/itsm_core/bootstrap/data/templates/*-management/docs/usecases/`）について、**OQ としての実施シナリオが存在する**ことを保証する。
...
- SSoT: `apps/itsm_core/bootstrap/data/templates/general-management/docs/usecases/03_risk_management.md.tpl`
...
- SSoT: `apps/itsm_core/bootstrap/data/templates/service-management/docs/usecases/19_retirement_and_migration.md.tpl`
...
- SSoT: `apps/itsm_core/bootstrap/data/templates/technical-management/docs/usecases/26_standardization.md.tpl`
