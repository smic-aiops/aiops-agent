...
- RLS: `apps/itsm_core/sor_ops/sql/itsm_sor_rls.sql` / `apps/itsm_core/sor_ops/sql/itsm_sor_rls_force.sql`
- 運用スクリプト（正）: `apps/itsm_core/sor_ops/scripts/`
- ワークフロー: `apps/itsm_core/sor_webhooks/workflows/` / `apps/itsm_core/<app>/workflows/`
- OQ: `apps/itsm_core/sor_ops/docs/itsm_core/oq/oq.md`
- CS: `apps/itsm_core/sor_ops/docs/itsm_core/cs/ai_behavior_spec.md`
...
- 改善機会の抽出（重複/取り漏れ/誤分類）
  - コントロール: ステータス（`状態/*`）の明確化、ラベルでのフィルタ（`ITSM/継続的改善` + `状態/Approved`）、抽出結果の監査可能性（Issue URL/ID を返す）

## 入口条件（Entry）

- `apps/itsm_core/sor_ops/sql/itsm_sor_core.sql` が SoR の正として管理されている
- 運用スクリプトが terraform output/SSM 経由で接続情報を解決できる（tfvars 直読みなし）

## 出口条件（Exit）

- IQ 合格: `apps/itsm_core/sor_ops/docs/itsm_core/iq/iq.md`
- OQ 合格: `apps/itsm_core/sor_ops/docs/itsm_core/oq/oq.md`
...
- CIR ステータス（`状態/*`）や CIR 抽出ルール（ラベル/対象プロジェクト）の変更
- CIR 起票テンプレ/marker および自動ラベル付与ルールの変更
- CIR クローズ運用（`状態/Closed`、note marker、クローズ条件/入力項目）の変更
