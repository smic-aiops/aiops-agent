...
本書は `apps/itsm_core/` の要求（What/Why）を定義します。詳細な利用方法・手順・実装は `apps/itsm_core/README.md` と `apps/itsm_core/sor_ops/docs/itsm_core/`、スキーマ（`apps/itsm_core/sor_ops/sql/`）、運用スクリプト（`apps/itsm_core/sor_ops/scripts/`）、ワークフロー（`apps/itsm_core/sor_webhooks/workflows/` および `apps/itsm_core/<app>/workflows/`）を正とします。
