...
# shellcheck source=apps/itsm_core/sor_ops/scripts/lib/itsm_db.sh
source "${REPO_ROOT}/apps/itsm_core/sor_ops/scripts/lib/itsm_db.sh"

usage() {
  cat <<'USAGE'
Usage:
  apps/itsm_core/sor_ops/scripts/apply_itsm_sor_retention.sh [options]

Purpose:
  - Apply ITSM SoR retention policy for a realm (dry-run first, then execute).

Options:
  --realm-key KEY          Target realm_key (default: default)
  --dry-run                Compute and print counts only (default)
  --execute                Execute deletions allowed by retention_policy
...
if [[ "${DRY_RUN}" == "true" ]]; then
  ...
  echo "[itsm] retention execute (realm_key=${REALM_KEY})"
fi
