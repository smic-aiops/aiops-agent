...
itsm_psql_local() {
  ...
  psql -X -q -P pager=off -h "${DB_HOST}" -p "${DB_PORT}" -U "${DB_USER}" -d "${DB_NAME}" -v ON_ERROR_STOP=1 "$@"
}
...
psql_base = f"PGPASSWORD=\"{password_ref}\" psql -X -q -P pager=off -h {shlex.quote(host)} -p {shlex.quote(port)} -U {shlex.quote(user)} -d {shlex.quote(db)} -v ON_ERROR_STOP=1"
