...
- n8n が稼働していること
- 環境変数（Zulip/GitLab など）は `apps/itsm_core/zulip_gitlab_issue_sync/README.md` を正とする
...
```bash
ACTIVATE=true apps/itsm_core/zulip_gitlab_issue_sync/scripts/deploy_workflows.sh
```

### 3. OQ 実行の準備（投稿の用意）

投稿条件（ボット投稿の扱い、`/oq-seed` 例外等）は `apps/itsm_core/zulip_gitlab_issue_sync/docs/oq/oq.md` を正とします。
