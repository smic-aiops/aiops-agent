...
**内容**
- 本アプリの仕様・運用・検証の入口を README に集約し、詳細は `apps/itsm_core/zulip_gitlab_issue_sync/docs/oq/` / `apps/itsm_core/zulip_gitlab_issue_sync/scripts/` を参照する。
...
- バリデーション成果物（最小）:
  - 本 README
  - OQ 文書: `apps/itsm_core/zulip_gitlab_issue_sync/docs/oq/oq.md` および `apps/itsm_core/zulip_gitlab_issue_sync/docs/oq/oq_*.md`（整備: `scripts/generate_oq_md.sh`）
  - OQ 実行補助: `apps/itsm_core/zulip_gitlab_issue_sync/scripts/run_oq.sh`
...
- 高レベル構成
  ...
  - （任意）→ S3
- スケジュール（既定）
  - `apps/itsm_core/zulip_gitlab_issue_sync/workflows/zulip_gitlab_issue_sync.json` の Cron で定期実行（既定: every minute）
- Webhook（OQ）
    ...
    - 前提: `apps/itsm_core/sql/itsm_sor_core.sql` を RDS に適用済みであること。
	    - 既存データのバックフィル（SoR を“正”として運用する場合は推奨）
	      ...
	      - GitLab Issue 全件 → SoR レコード backfill: `apps/itsm_core/gitlab_backfill_to_sor/workflows/gitlab_issue_backfill_to_sor.json`
	        ...
	        - スモークテスト（SoR への bulk upsert）: `apps/itsm_core/gitlab_backfill_to_sor/workflows/gitlab_issue_backfill_to_sor_test.json`（Webhook: `POST /webhook/gitlab/issue/backfill/sor/test`）
		      - GitLab の過去決定（Issue 本文/Note）: `apps/itsm_core/gitlab_backfill_to_sor/workflows/gitlab_decision_backfill_to_sor.json`
		        ...
		        - スモークテスト（SoR への bulk insert）: `apps/itsm_core/gitlab_backfill_to_sor/workflows/gitlab_decision_backfill_to_sor_test.json`（Webhook: `POST /webhook/gitlab/decision/backfill/sor/test`）
...
- ワークフロー: `apps/itsm_core/zulip_gitlab_issue_sync/workflows/gitlab_decision_notify.json`
...
- SoR（任意だが推奨）
  ...
  - 過去の GitLab 決定（Issue 本文/Note）は、必要に応じて `apps/itsm_core/gitlab_backfill_to_sor/workflows/gitlab_decision_backfill_to_sor.json` で SoR へバックフィルします（Webhook: `POST /webhook/gitlab/decision/backfill/sor`）。
...
### ディレクトリ構成
- `apps/itsm_core/zulip_gitlab_issue_sync/workflows/`: n8n ワークフロー（JSON）
- `apps/itsm_core/zulip_gitlab_issue_sync/scripts/`: n8n 同期（アップロード）・OQ 実行スクリプト
- `apps/itsm_core/zulip_gitlab_issue_sync/docs/cs/`: CS（Configuration Specification: 設計・構成定義）
- `apps/itsm_core/zulip_gitlab_issue_sync/docs/oq/`: OQ（運用適格性確認）
- `apps/itsm_core/zulip_gitlab_issue_sync/data/default/prompt/system.md`: サブアプリ単位の中心プロンプト（System 相当）
- `apps/itsm_core/zulip_gitlab_issue_sync/sql/`: 予約（必要に応じて補助 SQL を配置）

### 同期（n8n Public API へ upsert）
```bash
apps/itsm_core/zulip_gitlab_issue_sync/scripts/deploy_workflows.sh
```
...
- 代表ケースは `apps/itsm_core/zulip_gitlab_issue_sync/docs/oq/oq.md` と個別 OQ 文書で定義する。
...
**文書/手順（最小）**
- 同期: `apps/itsm_core/zulip_gitlab_issue_sync/scripts/deploy_workflows.sh`（`DRY_RUN=true` で差分確認）
...
**文書**
- `apps/itsm_core/zulip_gitlab_issue_sync/docs/oq/oq.md`（`oq_*.md` から生成）
- 個別シナリオ: `apps/itsm_core/zulip_gitlab_issue_sync/docs/oq/oq_*.md`

**実行**
- `apps/itsm_core/zulip_gitlab_issue_sync/scripts/run_oq.sh`

補足:
- OQ 実行前に `scripts/generate_oq_md.sh --app apps/itsm_core/zulip_gitlab_issue_sync` を実行し、`oq.md` の生成領域を最新化する
