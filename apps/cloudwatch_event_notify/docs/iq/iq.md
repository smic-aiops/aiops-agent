...
- CS（設計・構成定義）: `apps/cloudwatch_event_notify/docs/cs/ai_behavior_spec.md`
- OQ: `apps/cloudwatch_event_notify/docs/oq/oq_cloudwatch_event_notify.md`（互換: `apps/cloudwatch_event_notify/docs/oq/oq.md`）
...
```bash
DRY_RUN=true apps/cloudwatch_event_notify/scripts/deploy_workflows.sh
```

### 2. 同期（反映）

```bash
ACTIVATE=true TEST_WEBHOOK=true apps/cloudwatch_event_notify/scripts/deploy_workflows.sh
```
...
※ 実行例は OQ に集約（`apps/cloudwatch_event_notify/docs/oq/oq_cloudwatch_event_notify.md`）。
