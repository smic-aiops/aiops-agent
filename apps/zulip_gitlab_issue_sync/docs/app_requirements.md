...
本書は `apps/zulip_gitlab_issue_sync/` の要求（What/Why）を定義します。詳細な利用方法・手順・実装は `apps/zulip_gitlab_issue_sync/README.md` と `apps/zulip_gitlab_issue_sync/docs/`、ワークフロー定義、同期スクリプトを正とします。
...
本セクションは `apps/zulip_gitlab_issue_sync/docs/dq/dq.md` の設計スコープ/主要リスクを、運用上のユースケースへ落とし込んだものです。
...
- 出力: GitLab Issue 状態の更新、および Zulip への結果投稿
