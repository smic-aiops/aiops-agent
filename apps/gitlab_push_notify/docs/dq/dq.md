...
- 本 README: `apps/gitlab_push_notify/README.md`
- ワークフロー:
...
- GitLab webhook セットアップ: `apps/gitlab_push_notify/scripts/setup_gitlab_project_webhook.sh`
...
- Webhook（main/test）と必須 env が `apps/gitlab_push_notify/README.md` に明記されている
- OQ の必須ケース（secret/通知/テスト）が整理されている（`apps/gitlab_push_notify/docs/oq/oq.md`）

## 出口条件（Exit）

- IQ 合格: `apps/gitlab_push_notify/docs/iq/iq.md`
- OQ 合格: `apps/gitlab_push_notify/docs/oq/oq.md`（特に `/test` と secret 検証、通知成立）
