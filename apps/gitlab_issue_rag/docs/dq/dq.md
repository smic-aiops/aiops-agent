...
- `apps/gitlab_issue_rag/sql/gitlab_issue_rag.sql` の適用手順と前提が README に明記されている
- OQ シナリオ（取得/embedding/差分同期/観測性）が整理されている（`apps/gitlab_issue_rag/docs/oq/oq.md`）

## 出口条件（Exit）

- IQ 合格: `apps/gitlab_issue_rag/docs/iq/iq.md`
- OQ 合格: `apps/gitlab_issue_rag/docs/oq/oq.md` の必須シナリオ（pgvector test、GitLab 取得、embedding 任意）
