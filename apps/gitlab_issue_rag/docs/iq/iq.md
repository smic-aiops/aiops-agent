...
- DB 初期化: `apps/gitlab_issue_rag/sql/gitlab_issue_rag.sql`
- 同期スクリプト: `apps/gitlab_issue_rag/scripts/deploy_issue_rag_workflows.sh`
- OQ: `apps/gitlab_issue_rag/docs/oq/oq.md`
- CS: `apps/gitlab_issue_rag/docs/cs/ai_behavior_spec.md`

## 前提

- n8n が稼働し、n8n Public API が利用可能であること
- Postgres（pgvector）に接続でき、ワークフローの Postgres ノードが適切な資格情報を参照していること
- 環境変数は `apps/gitlab_issue_rag/README.md` を正とする

## テストケース一覧

| ID | 目的 | 実施 | 期待結果 |
| --- | --- | --- | --- |
| IQ-GIR-DB-001 | DB 初期化（初回のみ） | 目視/SQL | `apps/gitlab_issue_rag/sql/gitlab_issue_rag.sql` が適用済みである |
...
`apps/gitlab_issue_rag/sql/gitlab_issue_rag.sql` を適用します（接続情報は環境に依存）。

### 2. 同期（差分確認）

```bash
DRY_RUN=true apps/gitlab_issue_rag/scripts/deploy_issue_rag_workflows.sh
```

### 3. 同期（反映）

```bash
ACTIVATE=true apps/gitlab_issue_rag/scripts/deploy_issue_rag_workflows.sh
```
...
※ 具体的な期待結果と証跡は `apps/gitlab_issue_rag/docs/oq/oq.md`（特に `OQ-GIR-001`）を正とする。
