...
- 送信スタブ（証跡出力対応）: `apps/aiops_agent/adapter/scripts/send_stub_event.py`

証跡（request/response JSON）を保存する例:

```bash
mkdir -p evidence/oq
python3 apps/aiops_agent/adapter/scripts/send_stub_event.py --base-url "<adapter_base_url>" --source cloudwatch --scenario normal --evidence-dir evidence/oq
```

本書は、n8n 上の OQ 実行ワークフロー `apps/aiops_agent/orchestrator/workflows/aiops_oq_runner.json`（`aiops-oq-runner`）を使い、**外部PCからのローカル実行なし**で、AI Ops アダプターの受信口（`/ingest/{source}`）と後続処理（正規化/冪等性/Preview/enqueue/callback）を確認するためのシナリオ集です。

> `apps/aiops_agent/adapter/scripts/send_stub_event.py`（Python 送信スタブ）は「任意/補助」です。n8n だけで完結できない場合や、n8n に入れない状況の代替として末尾に残します。
...
- n8n に次のワークフローがインポートされていること
  - `apps/aiops_agent/adapter/workflows/aiops_adapter_ingest.json`（受信〜正規化〜Preview/enqueue 分岐）
  - `apps/aiops_agent/orchestrator/workflows/aiops_orchestrator.json`（`jobs.Preview` / `jobs.enqueue`）
- `apps/aiops_agent/execution_engine/workflows/aiops_job_engine_queue.json`（`jobs/enqueue` と Queue/Worker。参照実装は **stub 実行**）
  - `apps/aiops_agent/adapter/workflows/aiops_adapter_callback.json`（callback 受信〜結果通知）
  - `apps/aiops_agent/orchestrator/workflows/aiops_oq_runner.json`（OQ 実行）
- ContextStore スキーマが適用済みであること（`apps/aiops_agent/knowledge_store/sql/aiops_context_store.sql`）
- ワークフローが利用する Postgres Credential が、上記 DB を指していること

## 実行（推奨）

個別シナリオ（OQ-USECASE）の自動実行は `apps/aiops_agent/orchestrator/scripts/run_oq.sh` を正とする（既定で **個別シナリオを一括実行**し、evidence を保存する）。
...
本リポジトリの運用スクリプト（`apps/aiops_agent/orchestrator/scripts/deploy_workflows.sh` 等）は、n8n へアクセスするベース URL として `N8N_API_BASE_URL`（未指定時は `terraform output` の `service_urls.n8n` から ALB 5678 を前提に生成）を使用します。
...
- 証跡チェックリストは `apps/aiops_agent/orchestrator/docs/dq/dq.md` の「証跡チェックリスト（必須）」に従う
...
1. `bash apps/aiops_agent/orchestrator/scripts/run_oq_runner.sh --execute` を実行する
...
- Zulip → n8n → Zulip の応答確認: `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_10_zulip_primary_hello.md`
- Zulip topic context（短文時に同一 stream/topic の直近メッセージを取得）: `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_12_zulip_topic_context.md`
- Zulip quick/defer ルーティング（即時返信/遅延返信の切替）: `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_27_zulip_quick_defer_routing.md`

## 個別シナリオ別ドキュメント

- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_01_chat_request_normal.md`: チャット依頼の Preview / enqueue / callback 一連検証
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_02_monitoring_auto_reaction.md`: CloudWatch など監視通知の自動反応確認
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_03_feedback.md`: `aiops_job_feedback` と `aiops_context.status` の結果更新確認
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_04_enrichment.md`: `enrichment_plan` に従う RAG/CMDB/Runbook 情報収集
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_05_trace_id_propagation.md`: trace_id の Adapter→Orchestrator→JobEngine→Callback の連携確認
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_06_llm_provider_switch.md`: PLaMo/OpenAI 切替時も `jobs/preview` が成功すること
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_07_security_auth.md`: 不正署名/トークンで 401/403 を返し、胚後続処理が走らないこと
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_08_policy_context_guardrails.md`: `policy_context.rules/defaults/fallbacks` に従う語彙ガードとフォールバック
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_09_queue_mode_worker.md`: 参照実装のジョブキュー worker（Postgres キュー + Cron worker）が `queued→running→success/failed` を辿ること
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_10_zulip_primary_hello.md`: Zulip こんにちはで返信確認
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_11_intent_clarification.md`: 曖昧入力の意図確認（質問を 1〜2 個に絞る）
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_12_zulip_topic_context.md`: Zulip topic context（短文時の直近メッセージ付与）
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_13_zulip_conversation_continuity.md`: Zulip 会話継続（直近文脈を踏まえる）
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_14_style_tone_follow.md`: 口調/丁寧度（policy_context に追従）
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_15_uncertainty_and_evidence.md`: 不確実性の表明（根拠/不足データ/追加取得・提示提案）
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_16_chat_to_action_handoff.md`: 会話→運用アクション接続（preview→承認→実行）
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_17_topic_switch_context_split.md`: スレッド/トピック切替（文脈分離）
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_18_secret_handling_in_chat.md`: 機密の扱い（伏字/誘導、保存/投稿回避）
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_19_ja_quality_terms.md`: 日本語品質（専門用語の言い換え等）
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_20_spam_burst_dedup.md`: 連投耐性（重複排除＋順序整合）
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_21_demo_sulu_night_misoperation_autorecovery.md`: 統合デモ（夜間の誤停止→自動復旧）
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_25_smalltalk_free_chat.md`: 雑談/世間話（`reply_only`）
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_26_ai_node_summary_output.md`: AIノードモニタリング（Sulu 管理画面で判断要約を確認）
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_27_zulip_quick_defer_routing.md`: Zulip quick/defer ルーティング（即時返信/遅延返信の切替）
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_28_approval_link_decision_history.md`: 承認リンク（クリック）を決定として扱い、履歴参照できること
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_29_auto_approval_as_decision.md`: 自動承認（auto_enqueue）を決定として扱い、履歴参照できること
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_30_cir_request_intake_and_notify.md`: 改善要望（CIR）の集約と、承認時の依頼者通知
...
- 手順（例）：
  - JobEngine の Cron worker を稼働させる（`apps/aiops_agent/execution_engine/workflows/aiops_job_engine_queue.json`）
...
`apps/aiops_agent/orchestrator/docs/oq/oq_usecase_26_ai_node_summary_output.md` を対象に、次を確認します。
...
`apps/aiops_agent/adapter/scripts/send_stub_event.py` は、外部から ingest を叩くための送信スタブです。n8n 上で完結できる場合は不要です。

- 例：任意テキストでチャット依頼を作りたい（`--text`）
- 例：CloudWatch のアラーム名を差し替えたい（`--cloudwatch-alarm-name`）

詳細は `python3 apps/aiops_agent/adapter/scripts/send_stub_event.py --help` を参照してください。
...
#### 手順
1. `apps/aiops_agent/adapter/scripts/send_stub_event.py --base-url "$N8N_ADAPTER_BASE_URL" --source slack --scenario normal --evidence-dir evidence/oq/oq_usecase_01_chat_request_normal`
2. `apps/aiops_agent/orchestrator/docs/oq/oq.md` の `trace_id` を確認し、`aiops_context` / `aiops_job_queue` などを SQL で抽出する（`aiops_context` の `reply_target`/`normalized_event` も確認）
...
- チャットプロンプト: `apps/aiops_agent/orchestrator/data/default/prompt/aiops_chat_core_ja.txt`
...
#### 前提
- `apps/aiops_agent/adapter/workflows/aiops_adapter_ingest.json` で `ingest/cloudwatch` Webhook が有効
...
#### 前提
- `apps/aiops_agent/adapter/workflows/aiops_adapter_feedback.json` が同期済みで `feedback` Webhook が活性
...
- `apps/aiops_agent/adapter/scripts/send_stub_event.py --source feedback` または手動 HTTP POST（例：`curl` で `/webhook/feedback/job`）
...
2. `apps/aiops_agent/adapter/scripts/send_stub_event.py` または `curl` で `POST $N8N_ADAPTER_BASE_URL/feedback/job` を送信
...
#### 関連
- `apps/aiops_agent/orchestrator/docs/oq/oq.md`
- `apps/aiops_agent/adapter/workflows/aiops_adapter_feedback.json`
...
#### 関連
- `apps/aiops_agent/orchestrator/docs/oq/oq.md`（RAG/CMDB/Runbook 検証）
- `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json`（`enrichment_plan` の defaults）
...
- `apps/aiops_agent/orchestrator/docs/oq/oq.md` の `証跡` で定義した `trace_id` 相関が成立
...
```bash
# dry-run（実行はしない）
apps/aiops_agent/orchestrator/scripts/run_oq_usecase_05_trace_id_propagation.sh
...
#### 関連
- `apps/aiops_agent/orchestrator/docs/oq/oq.md`（trace_id 相関章）
...
- `apps/aiops_agent/orchestrator/workflows/aiops_orchestrator.json` が `rag_router` で適切な prompt を選び、`next_action` を再現できる
...
#### 関連
- `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json`（`llm_provider` defaults/fallbacks）
- `apps/aiops_agent/orchestrator/docs/oq/oq.md`（`rag_route`/`prompt_history` 章）
...
#### 前提
- `apps/aiops_agent/execution_engine/workflows/aiops_job_engine_queue.json` の Cron worker が起動済み（`triggerTimes.everyMinute` 等）
...
4. `aiops_adapter_callback` で `callback.job_id`/`status` と `apps/aiops_agent/orchestrator/scripts/run_iq_tests_aiops_agent.sh` の `callback` 結果が一致するか確認
...
#### 関連
- `apps/aiops_agent/execution_engine/workflows/aiops_job_engine_queue.json`
- `apps/aiops_agent/orchestrator/docs/oq/oq.md`
...
- 投稿の起点は `apps/aiops_agent/orchestrator/scripts/run_oq_zulip_primary_hello.sh`（実行時にレルム/stream/topic は引数や環境変数で解決される）
...
```bash
bash apps/aiops_agent/orchestrator/scripts/run_oq_zulip_primary_hello.sh
```
...
```bash
bash apps/aiops_agent/orchestrator/scripts/run_oq_zulip_primary_hello.sh --execute --evidence-dir evidence/oq/oq_zulip_primary_hello
```
...
- `apps/aiops_agent/orchestrator/scripts/run_oq_zulip_primary_hello.sh` の実行ログ
...
- `apps/aiops_agent/orchestrator/docs/oq/oq.md`
- `apps/aiops_agent/orchestrator/docs/zulip_chat_bot.md`
...
#### 関連
- `apps/aiops_agent/orchestrator/docs/oq/oq.md`
- `apps/aiops_agent/orchestrator/docs/aiops_agent_specification.md`
...
- OQ runner が利用できる（`apps/aiops_agent/orchestrator/scripts/run_oq_runner.sh`）
...
```bash
apps/aiops_agent/orchestrator/scripts/run_oq_runner.sh --execute --evidence-dir evidence/oq/oq_zulip_topic_context
```
...
- topic context 付与（短文時の直近メッセージ取得）が有効
  - 参考: `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_12_zulip_topic_context.md`
- OQテスト時は `N8N_DEBUG_LOG=true` を環境変数で有効化する（デフォルトは `false`）
...
#### 関連
- `apps/aiops_agent/orchestrator/docs/oq/oq.md`
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_12_zulip_topic_context.md`
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_10_zulip_primary_hello.md`
...
- `apps/aiops_agent/orchestrator/docs/oq/oq.md`
- `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json`
- `apps/aiops_agent/orchestrator/data/default/prompt/initial_reply_ja.txt`
...
#### 関連
- `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json`
- `apps/aiops_agent/orchestrator/data/default/prompt/jobs_preview_ja.txt`
- `apps/aiops_agent/orchestrator/data/default/prompt/initial_reply_ja.txt`
...
#### 関連
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_01_chat_request_normal.md`
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_07_security_auth.md`
...
#### 関連
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_12_zulip_topic_context.md`
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_13_zulip_conversation_continuity.md`
...
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_07_security_auth.md`
- `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json`
- `apps/aiops_agent/orchestrator/data/default/prompt/interaction_parse_ja.txt`
...
#### 関連
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_15_uncertainty_and_evidence.md`
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_12_zulip_topic_context.md`
...
#### 関連
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_01_chat_request_normal.md`
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_05_trace_id_propagation.md`
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_17_topic_switch_context_split.md`
...
#### 関連
- `apps/aiops_agent/orchestrator/docs/cs/ai_behavior_spec.md`
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_02_monitoring_auto_reaction.md`
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_04_enrichment.md`
- `apps/workflow_manager/service_request/workflows/aiops_sulu_service_control.json`
...
```bash
# ドライラン（解決値の確認）
bash apps/aiops_agent/orchestrator/scripts/run_oq_zulip_primary_hello.sh --message "今日は寒いですね"

# 実行（送信 + 返信待ち）
bash apps/aiops_agent/orchestrator/scripts/run_oq_zulip_primary_hello.sh --execute --evidence-dir evidence/oq/oq_usecase_25_smalltalk --message "今日は寒いですね"
```
...
```bash
bash apps/aiops_agent/orchestrator/scripts/run_oq_zulip_primary_hello.sh --execute --evidence-dir evidence/oq/oq_usecase_27_zulip_quick_defer --message "今日は寒いですね"
```

3. defer の確認（送信 + 返信待ち）

```bash
bash apps/aiops_agent/orchestrator/scripts/run_oq_zulip_primary_hello.sh --execute --evidence-dir evidence/oq/oq_usecase_27_zulip_quick_defer --message "今日の最新のAWS障害情報をWeb検索して教えて"
```
...
- `apps/aiops_agent/orchestrator/docs/zulip_chat_bot.md`
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_10_zulip_primary_hello.md`
- `apps/aiops_agent/orchestrator/docs/oq/oq_usecase_25_smalltalk_free_chat.md`
- `apps/aiops_agent/orchestrator/docs/oq/oq.md`
...
### OQ-USECASE-29: 自動承認（auto_enqueue）を決定として扱い、履歴を参照できる（source: `oq_usecase_29_auto_approval_as_decision.md`）
...
### OQ-USECASE-30: 改善要望（CIR）を集約し、承認時に依頼者へ通知できる（source: `oq_usecase_30_cir_request_intake_and_notify.md`）
