...
- 設計書: `apps/aiops_agent/orchestrator/docs/aiops_agent_design.md`
- 仕様書/要求: `apps/aiops_agent/orchestrator/docs/aiops_agent_specification.md` / `apps/aiops_agent/orchestrator/docs/app_requirements.md`
- 運用ガイド: `apps/aiops_agent/orchestrator/docs/aiops_agent_usage.md`
...
- `dq_run_id` と証跡保存先（`apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json` の `dq.evidence_dir_template`）が確定している

## 出口条件（Exit）

- 必須シナリオが全件合格し、全シナリオの合格率が `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json` の `dq.scenario_pass_rate_min` 以上
- critical シナリオの合格率が `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json` の `dq.critical_scenario_pass_rate_min` を満たす
- JSON 妥当性率が `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json` の `dq.json_validity_rate_min` 以上、PII 逸脱が `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json` の `dq.pii_violation_max` 以下
- 実運用指標が `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json` の `dq` を満たす
...
- ポリシーは `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json` の `version` を参照する
...
| 指標 | 定義 | 目標値（正） |
| --- | --- | --- |
| 誤判定率（False Positive Rate） | 承認/自動投入が運用上の差戻しで覆った割合（レビュー結果に基づく） | `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json` の `dq.false_positive_rate_pct_max` 以下 |
| 未解決率 | `aiops_job_feedback.resolved=false` / `aiops_job_feedback` 総数 | `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json` の `dq.unresolved_rate_pct_max` 以下 |
| プレビュー評価スコア | `aiops_preview_feedback.score` の平均 | `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json` の `dq.preview_score_min` 以上 |
| 手動トリアージ率 | `normalized_event.needs_manual_triage=true` / 全件 | `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json` の `dq.manual_triage_rate_pct_max` 以下 |
| API レイテンシ/応答成功率 | LLM 呼び出しの p95 / エラー率 | `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json` の `dq.llm_latency_ms_p95_max` 以下 / `dq.llm_error_rate_pct_max` 以下 |
| JSON 妥当性率 | JSON 解析成功かつ必須キー/語彙が有効な件数 / 全 LLM 出力件数 | `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json` の `dq.json_validity_rate_min` 以上 |
| PII 逸脱件数 | `pii_handling` 違反検知件数 | `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json` の `dq.pii_violation_max` 以下 |

> 指標の SSoT は `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json` の `dq` とする。
...
2. **運用指標閾値**: 直近 7 日平均が `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json` の `dq` を超える場合は次環境へ進まない
3. **IQ/oq/PQ の合格**: `apps/aiops_agent/orchestrator/docs/iq/iq.md` / `apps/aiops_agent/orchestrator/docs/oq/oq.md` / `apps/aiops_agent/orchestrator/docs/pq/pq.md` の必須項目が合格
...
集計期間の既定は `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json` の `dq.metric_window_days_default` を正とする。

```bash
bash apps/aiops_agent/orchestrator/scripts/run_dq_llm_quality_report.sh --since-days 7
```
...
```bash
bash apps/aiops_agent/orchestrator/scripts/run_dq_llm_quality_report.sh --since-days 7 --format json --output evidence/dq/<dq_run_id>/llm_quality_report.json
```

### IQ（疎通/連携）

```bash
bash apps/aiops_agent/orchestrator/scripts/run_iq_tests_aiops_agent.sh
```

### OQ/PQ（運用/性能）

`apps/aiops_agent/orchestrator/docs/oq/oq.md` / `apps/aiops_agent/orchestrator/docs/pq/pq.md` の手順に従い、証跡を保存する。
...
- 直近実施との差分で以下を満たすこと
  - critical 合格率の低下は `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json` の `dq.degradation.critical_pass_rate_drop_max_pct` 以内
  - JSON 妥当性率の低下は `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json` の `dq.degradation.json_validity_drop_max_pct` 以内
  - PII 逸脱は `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json` の `dq.degradation.pii_violation_increase_max` 以内
  - 実運用指標の悪化は `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json` の `dq.degradation.ops_metric_degrade_max_pct` 以内
