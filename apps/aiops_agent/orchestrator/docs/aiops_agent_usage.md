...
- `WORKFLOW_DIR_AGENT`: upsert 対象ディレクトリ（カンマ/スペース区切りで複数指定可。既定は `apps/aiops_agent/{adapter,orchestrator,execution_engine,knowledge_store}/workflows`）
...
- `N8N_GITLAB_ESCALATION_MD_PATH`: エスカレーション MD パス（例: `<path/to/escalation_matrix.md>`、テンプレ: `apps/aiops_agent/orchestrator/docs/templates/escalation_matrix.md`）
- `N8N_GITLAB_ESCALATION_MD_PATH_<REALM>`: レルム別 MD パス
- `N8N_GITLAB_WORKFLOW_CATALOG_MD_PATH`: ワークフローカタログ MD パス（例: `<path/to/workflow_catalog.md>`、テンプレ: `apps/aiops_agent/orchestrator/docs/templates/workflow_catalog.md`）
...
- スクリプト: `apps/aiops_agent/orchestrator/scripts/setup_aiops_agent.sh`
...
```bash
# dry-run
apps/aiops_agent/orchestrator/scripts/setup_aiops_agent.sh

# 実行（SSM 参照 + n8n 参照）
apps/aiops_agent/orchestrator/scripts/setup_aiops_agent.sh --execute
...
- ワークフロー: `apps/aiops_agent/knowledge_store/workflows/aiops_problem_management_sync.json`
- 設定 MD: `apps/aiops_agent/orchestrator/docs/templates/problem_management_sync.md`
- `N8N_GITLAB_PROBLEM_CONFIG_MD_PATH`: 問題管理同期の設定 MD（既定テンプレの配置パス）
- 同期対象フィルタは MD の `issue_filters` と一致させる（テンプレと運用設定の二重管理を避ける）

## 変更時の DQ/IQ/OQ/PQ

- DQ の正: `apps/aiops_agent/orchestrator/docs/dq/dq.md`
- DQ 指標: `apps/aiops_agent/orchestrator/scripts/run_dq_llm_quality_report.sh`
- IQ: `apps/aiops_agent/orchestrator/scripts/run_iq_tests_aiops_agent.sh`
- OQ/PQ: `apps/aiops_agent/orchestrator/docs/oq/oq.md` / `apps/aiops_agent/orchestrator/docs/pq/pq.md` の手順に従う
- 証跡は `aiops_prompt_history`、n8n 実行履歴、DB 集計結果を揃える

## DQ/IQ/OQ/PQ の実施

- DQ: `apps/aiops_agent/orchestrator/docs/dq/dq.md` の基準に従い、`dq_run_id` と証跡（`evidence/dq/<dq_run_id>/`）を保存する
- IQ: `apps/aiops_agent/orchestrator/docs/iq/iq.md` の結果ファイルを証跡として保存する
- OQ: `apps/aiops_agent/orchestrator/docs/oq/oq.md` の `trace_id` と実行ログ/DB 証跡を紐付ける
- PQ: `apps/aiops_agent/orchestrator/docs/pq/pq.md` の計測結果（ダッシュボード/集計）を保存する
...
- DQ は `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json` の `dq.degradation` に基づきデグレ判定を行う
...
- **入力パラメータ（例）**:
  ...
  - `source`: 受信ポリシー（facts）`apps/aiops_agent/orchestrator/data/default/policy/ingest_policy_ja.json` の `sources` に含まれる値
  - `scenario`: シナリオ定義（facts）`apps/aiops_agent/adapter/scripts/stub_scenarios.json` の `scenarios` に含まれる値
...
- 送信スタブ（証跡出力対応）：`apps/aiops_agent/adapter/scripts/send_stub_event.py`
  - シナリオ定義（facts）：`apps/aiops_agent/adapter/scripts/stub_scenarios.json`
- OQ（運用適格性確認）シナリオ集：`apps/aiops_agent/orchestrator/docs/oq/oq.md`
...
Zulip 連携の要求・仕様・実装・送信スタブは `apps/aiops_agent/orchestrator/docs/zulip_chat_bot.md` を参照する。
...
DQ は `apps/aiops_agent/orchestrator/docs/dq/dq.md` を正とし、設計変更時は次を満たすこと。
