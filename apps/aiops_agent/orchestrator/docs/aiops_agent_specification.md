...
* **required_confirm**：後方互換のフラグ（`next_action` を正とし、語彙や扱いは `apps/aiops_agent/orchestrator/data/default/prompt/jobs_preview_ja.txt` のポリシーに従う）。本文では `required_confirm` の意味論を固定しない。
...
本節は **AIOps Agent（アダプター）とソース間**の接続/通信を一覧化します。ここでいう **ソース名** は `apps/aiops_agent/orchestrator/data/default/policy/ingest_policy_ja.json` の `sources` キー（例: `zulip`, `slack`）を正とします。
...
- 受信は `POST /ingest/{source}` を基本とし、具体の受信パス（例: Zulip は `/ingest/zulip`）は各ソース連携仕様を正とします（Zulip は `apps/aiops_agent/orchestrator/docs/zulip_chat_bot.md`）。
- 冪等化（`dedupe_key`）の算出に用いる `event_id` 抽出は `apps/aiops_agent/orchestrator/data/default/policy/ingest_policy_ja.json`（`event_id_paths`）を正とします。
...
* LLM 入出力は `prompt_key` と JSON Schema を 1 対 1 で紐付け、スキーマは `apps/aiops_agent/{adapter,orchestrator}/schema/<prompt_key>.{input,output}.json` としてバージョン管理する（整合性チェック: `apps/aiops_agent/orchestrator/scripts/validate_llm_schemas.sh`）。
...
Zulip 連携の要求・仕様・実装は `apps/aiops_agent/orchestrator/docs/zulip_chat_bot.md` に集約します。
...
設計本文に具体値が出てくる場合も、運用時の正は `apps/aiops_agent/orchestrator/data/default/policy/*.json` と各プロンプト本文であり、コードや本文の固定値で判断を上書きしない（レルム別上書きが必要な場合は `apps/aiops_agent/orchestrator/data/<realm>/{policy,prompt}/`）。
本文の具体値/順序/例外は、`policy_context.rules.common.policy_ownership` を正とし、実装時は `policy_context` とプロンプトの更新で反映する。

参照実装（本リポジトリ）:

* 承認ポリシー（事実データ）：`apps/aiops_agent/orchestrator/data/default/policy/approval_policy_ja.json`
* 意思決定ポリシー（上限/語彙/既定値/条件分岐/フォールバックなど）：`apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json`
* ソース別の入力制約（facts）：`apps/aiops_agent/orchestrator/data/default/policy/source_capabilities_ja.json`

### 2.10 ルール所在マップ（Single Source of Truth）

本文に現れる条件分岐/語彙/上限/テンプレートは参照用の例とし、運用時の正は次のポリシー/プロンプトに集約します。

* 共通ガードレール/証拠範囲/言語方針/役割制約：`apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json`（`rules.common`）
* 分類/優先度/意図解析：`apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json`（`rules.adapter_classify`/`taxonomy`）＋ Unified Decision（論理）に属するプロンプト群（物理は用途別ノード。`adapter.interaction_parse.v1` / `adapter.classify.v1`）
* ルーティング/エスカレーション：`apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json`（`rules.routing_decide`）＋`apps/aiops_agent/orchestrator/data/default/prompt/routing_decide_ja.txt`
* 承認要否/候補選定/初動返信：`apps/aiops_agent/orchestrator/data/default/policy/approval_policy_ja.json`＋`apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json`（`rules.jobs_preview`/`rules.initial_reply`）＋ Unified Decision（論理）に属するプロンプト群（`orchestrator.preview.v1` / `adapter.initial_reply.v1`）
* 結果通知/評価依頼：`apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json`（`rules.job_result_reply`/`rules.feedback_request_render`）＋各プロンプト
* 承認/評価の文法：`apps/aiops_agent/orchestrator/data/default/policy/interaction_grammar_ja.json`
* 受信/冪等化：`apps/aiops_agent/orchestrator/data/default/policy/ingest_policy_ja.json`
* ソース別の入力制約：`apps/aiops_agent/orchestrator/data/default/policy/source_capabilities_ja.json`
* PQ/送信スタブの期待値：`apps/aiops_agent/adapter/scripts/stub_scenarios.json`
...
* `policy_context.version` は `apps/aiops_agent/orchestrator/data/default/policy/decision_policy_ja.json` の `version` と同期し、LLM 入力/出力・コンテキストストア・監査ログに同じ文字列をそのまま流します。`aiops_prompt_history` の新設列 `policy_version` にはこの値を書き出し、`prompt_hash` と併せて「どの判断ポリシーとプロンプトでその応答が作られたか」が追跡できるようにします。
