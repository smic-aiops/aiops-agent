...
usage() {
  cat <<'USAGE'
Usage: apps/aiops_agent/orchestrator/scripts/run_oq.sh [options]
...
main() {
  ...
  run_step "oq-usecase-05" \
    bash apps/aiops_agent/orchestrator/scripts/run_oq_usecase_05_trace_id_propagation.sh --execute --realm "${REALM}" ${N8N_URL_ARGS[@]+"${N8N_URL_ARGS[@]}"} --evidence-dir "${EVIDENCE_DIR}/oq_05"

  # 1) OQ runner (bulk ingest/pattern cases)
  run_step "oq-runner" \
    bash apps/aiops_agent/orchestrator/scripts/run_oq_runner.sh --execute --realm "${REALM}" ${N8N_URL_ARGS[@]+"${N8N_URL_ARGS[@]}"} --evidence-dir "${EVIDENCE_DIR}/oq_runner"

  # 2) Usecase 10 (Zulip primary hello)
  run_step "oq-usecase-10" \
    bash apps/aiops_agent/orchestrator/scripts/run_oq_zulip_primary_hello.sh --execute --realm "${REALM}" --evidence-dir "${EVIDENCE_DIR}/oq_10"

  # 3) Usecase 13 (Zulip conversation continuity)
  run_step "oq-usecase-13" \
    bash apps/aiops_agent/orchestrator/scripts/run_oq_zulip_conversation_continuity.sh --execute --realm "${REALM}" --evidence-dir "${EVIDENCE_DIR}/oq_13"

  # 4) Usecase 02/03/11/12 (monitoring/feedback/intent/topic-context)
  run_step "oq-usecases-02-03-11-12" \
    bash apps/aiops_agent/orchestrator/scripts/run_oq_usecases_02_03_11_12.sh --execute --realm "${REALM}" ${N8N_URL_ARGS[@]+"${N8N_URL_ARGS[@]}"} --evidence-dir "${EVIDENCE_DIR}/oq_02_03_11_12"

  # 4) Usecase 08 (policy_context guardrails)
  run_step "oq-usecase-08" \
    bash apps/aiops_agent/orchestrator/scripts/run_oq_usecase_08_policy_context_guardrails.sh --execute --realm "${REALM}" ${N8N_URL_ARGS[@]+"${N8N_URL_ARGS[@]}"} --evidence-dir "${EVIDENCE_DIR}/oq_08"

  # 5) Usecases 14-19 (Zulip quality)
  run_step "oq-usecases-14-19" \
    bash apps/aiops_agent/orchestrator/scripts/run_oq_usecases_14_19_zulip_quality.sh --execute --realm "${REALM}" --evidence-dir "${EVIDENCE_DIR}/oq_14_19"

  # 6) Usecase 20 (spam burst dedupe, DB-check)
  # This helper uses N8N_URL env instead of --n8n-url.
  if [[ -n "${N8N_URL}" ]]; then
    run_step "oq-usecase-20" \
      env N8N_URL="${N8N_URL}" bash apps/aiops_agent/orchestrator/scripts/run_oq_usecase_20_spam_burst_dedup_db.sh --execute --evidence-dir "${EVIDENCE_DIR}/oq_20"
  else
    run_step "oq-usecase-20" \
      bash apps/aiops_agent/orchestrator/scripts/run_oq_usecase_20_spam_burst_dedup_db.sh --execute --evidence-dir "${EVIDENCE_DIR}/oq_20"
  fi

  # 7) Usecase 21 (demo: Sulu misoperation autorecovery)
  run_step "oq-usecase-21" \
    bash apps/aiops_agent/orchestrator/scripts/run_oq_usecase_21_demo_sulu_night_misoperation_autorecovery.sh --execute --realm "${REALM}" ${N8N_URL_ARGS[@]+"${N8N_URL_ARGS[@]}"} --evidence-dir "${EVIDENCE_DIR}/oq_21"

  # 8) Usecase 25 (smalltalk / reply_only)
  run_step "oq-usecase-25" \
    bash apps/aiops_agent/orchestrator/scripts/run_oq_usecase_25_smalltalk_free_chat.sh --execute --realm "${REALM}" --evidence-dir "${EVIDENCE_DIR}/oq_25"

  # 9) Usecase 27 (quick/defer routing)
  run_step "oq-usecase-27" \
    bash apps/aiops_agent/orchestrator/scripts/run_oq_usecase_27_zulip_quick_defer_routing.sh --execute --realm "${REALM}" --evidence-dir "${EVIDENCE_DIR}/oq_27"
