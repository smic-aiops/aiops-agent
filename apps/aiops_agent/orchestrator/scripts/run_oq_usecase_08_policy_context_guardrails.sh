...
usage() {
  cat <<'USAGE'
Usage:
  apps/aiops_agent/orchestrator/scripts/run_oq_usecase_08_policy_context_guardrails.sh [options]
...
main() {
  ...
  if [[ "${DRY_RUN}" == "1" ]]; then
    ...
    apps/aiops_agent/knowledge_store/scripts/query_context_db_via_n8n.sh --trace-id "TRACE_ID_PLACEHOLDER" --evidence-dir "${evidence_root}/db" >/dev/null || true
    exit 0
  fi
  ...
  local db_json
  db_json="$(
    apps/aiops_agent/knowledge_store/scripts/query_context_db_via_n8n.sh \
      --execute \
      --trace-id "${trace_id}" \
