...
#   WORKFLOW_DIR              : deprecated (use WORKFLOW_DIR_AGENT); default is all component workflow dirs
#   WORKFLOW_DIR_AGENT        : workflow dir(s) to upsert (comma/space-separated). default is all component workflow dirs
...
#   N8N_VALIDATE_LLM_SCHEMAS : "true" (default) to validate LLM schemas against workflow prompt_key usage
...
#   N8N_REALM_DATA_DIR_BASE : base dir for realm-specific prompt/policy overrides (default: apps/aiops_agent/orchestrator/data)
...
if is_truthy "${VALIDATE_LLM_SCHEMAS}"; then
  if is_truthy "${DRY_RUN}"; then
    bash "${REPO_ROOT}/apps/aiops_agent/orchestrator/scripts/validate_llm_schemas.sh" --dry-run
  else
    bash "${REPO_ROOT}/apps/aiops_agent/orchestrator/scripts/validate_llm_schemas.sh"
  fi
fi
...
N8N_REALM_DATA_DIR_BASE="${N8N_REALM_DATA_DIR_BASE:-apps/aiops_agent/orchestrator/data}"
...
parse_workflow_dir_list() {
  ...
  for part in "${parts[@]}"; do
    if [[ -n "${part}" ]]; then
      append_unique_workflow_dir "${part}"
    fi
  done
}

WORKFLOW_DIR_DEFAULTS="apps/aiops_agent/adapter/workflows,apps/aiops_agent/orchestrator/workflows,apps/aiops_agent/execution_engine/workflows,apps/aiops_agent/knowledge_store/workflows"
WORKFLOW_DIR="${WORKFLOW_DIR:-${WORKFLOW_DIR_DEFAULTS}}"
WORKFLOW_DIR_AGENT="${WORKFLOW_DIR_AGENT:-${WORKFLOW_DIR}}"
WORKFLOW_DIRS=()
parse_workflow_dir_list "${WORKFLOW_DIR_AGENT}"
...
sync_workflows_for_realm() {
  ...
  if [[ -n "${realm_overlay_dir}" && -d "${realm_overlay_dir}/data" ]]; then
    ...
    # Backward-compatible: <N8N_REALM_DATA_DIR_BASE>/<realm>/*
