...
WORKFLOW_DIR="apps/aiops_agent/adapter/workflows,apps/aiops_agent/orchestrator/workflows,apps/aiops_agent/execution_engine/workflows,apps/aiops_agent/knowledge_store/workflows"
DRY_RUN=1
FORCE=0
...
workflow_dirs = [Path(p.strip()) for p in str(workflow_dir).split(",") if p.strip()]
...
for d in workflow_dirs:
    workflow_files.extend(list(d.glob("*.json")))
workflow_files = sorted(set(workflow_files))
if not workflow_files:
    print(f\"No workflow files found under: {', '.join(str(p) for p in workflow_dirs)}\")
    raise SystemExit(1)
