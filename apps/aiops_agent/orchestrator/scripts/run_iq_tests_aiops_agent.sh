...
if [[ "${RUN_INGEST_TESTS}" == "true" ]]; then
  set +e
  INGEST_OUTPUT="$(python3 apps/aiops_agent/adapter/scripts/send_stub_event.py --base-url "${N8N_WEBHOOK_BASE_URL}" --source cloudwatch --scenario normal --timeout-sec 15 2>&1)"
  INGEST_STATUS="$?"
  set -e
