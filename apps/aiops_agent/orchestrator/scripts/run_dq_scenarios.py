...
def repo_root() -> Path:
    # __file__ = apps/aiops_agent/orchestrator/scripts/run_dq_scenarios.py
    # parents[4] = repo root
    return Path(__file__).resolve().parents[4]


def default_scenarios_path() -> Path:
    return repo_root() / "apps" / "aiops_agent" / "orchestrator" / "data" / "default" / "dq" / "scenarios" / "representative_scenarios.json"


def policy_dir() -> Path:
    # Prefer the SSoT under apps/aiops_agent/orchestrator/data/default/policy/.
    # Keep backward compatibility if a legacy apps/aiops_agent/policy/ exists.
    legacy = repo_root() / "apps" / "aiops_agent" / "policy"
    if legacy.exists():
        return legacy
    return repo_root() / "apps" / "aiops_agent" / "orchestrator" / "data" / "default" / "policy"
