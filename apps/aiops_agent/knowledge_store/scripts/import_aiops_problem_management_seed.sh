...
usage() {
  cat <<'USAGE'
Usage:
  apps/aiops_agent/knowledge_store/scripts/import_aiops_problem_management_seed.sh [options]

Options:
  --schema PATH           Schema SQL to apply (default: apps/aiops_agent/knowledge_store/schema/aiops_problem_management.sql)
  --seed PATH             Seed SQL to apply (default: apps/aiops_agent/knowledge_store/schema/aiops_problem_management_seed.sql)
...
SCHEMA_FILE="${SCHEMA_FILE:-apps/aiops_agent/knowledge_store/schema/aiops_problem_management.sql}"
SEED_FILE="${SEED_FILE:-apps/aiops_agent/knowledge_store/schema/aiops_problem_management_seed.sql}"
APPLY_SCHEMA="true"
APPLY_SEED="true"
...
if [[ "${LOCAL_PSQL}" == "true" ]]; then
  ...
  if manage_local_psql; then
    echo "Problem management seed import completed (local)."
    exit 0
  else
    echo "Problem management seed import failed locally."
    exit 1
  fi
fi
...
if apply_schema:
    parts.append(f"printf %s {shlex.quote(schema_b64)} | base64 -d > /tmp/aiops_problem_schema.sql")
if apply_seed:
    parts.append(f"printf %s {shlex.quote(seed_b64)} | base64 -d > /tmp/aiops_problem_seed.sql")
...
if apply_schema:
    parts.append(f"{psql_base} -f /tmp/aiops_problem_schema.sql")
if apply_seed:
    parts.append(f"{psql_base} -f /tmp/aiops_problem_seed.sql")
parts.append("rm -f /tmp/aiops_problem_schema.sql /tmp/aiops_problem_seed.sql")
...
echo "Problem management seed import completed."
