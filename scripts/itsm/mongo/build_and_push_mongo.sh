...
main() {
  ...
  local src="public.ecr.aws/docker/library/mongo:${MONGO_TAG}"
  ...
  docker_use_local_or_load_cache "mongo" "${src}" "${tar_path}" "run scripts/itsm/mongo/pull_mongo_image.sh"
  docker tag "${src}" "${ecr_uri}:${MONGO_TAG}"
