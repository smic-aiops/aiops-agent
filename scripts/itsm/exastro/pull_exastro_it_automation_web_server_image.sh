...
pull_and_tag() {
  ...
  if is_truthy "${DRY_RUN}"; then
    ...
    echo "[exastro-web] [dry-run] docker tag \"${src}\" \"${dst}\""
    return 0
  fi
...
extract_fs() {
  ...
  if is_truthy "${DRY_RUN}"; then
    echo "[exastro-web] [dry-run] would export filesystem of ${tag} into ${outdir}"
    echo "[exastro-web] [dry-run] docker create \"${tag}\""
    echo "[exastro-web] [dry-run] docker export <cid> | ${TAR_BIN} ... \"${outdir}\" -xf -"
    echo "[exastro-web] [dry-run] docker rm <cid>"
    return 0
  fi
  if is_truthy "${CLEAN_CACHE}"; then
    ...
    echo "[exastro-web] Preserving existing cache dir: ${outdir}"
    return 0
  fi
  make_tree_writable "${outdir}"
  rm -rf "${outdir}"
...
main() {
  if is_truthy "${DRY_RUN}"; then
    echo "[exastro-web] Dry run enabled; no changes will be made."
    echo "[exastro-web] [dry-run] mkdir -p \"${LOCAL_IMAGE_DIR}\""
  else
