...
usage() {
  cat <<'USAGE'
Usage:
  scripts/itsm/odoo/build_and_push_odoo.sh [--dry-run]
...
Environment overrides:
  ...
  ECR_REPO_ODOO  (default: terraform output ecr_repo_odoo, fallback odoo)
...
if [ -z "${ECR_REPO_ODOO:-}" ]; then
  ECR_REPO_ODOO="$(tf_output_raw ecr_repo_odoo 2>/dev/null || echo "odoo")"
fi
...
if [ -z "${ODOO_IMAGE_TAG:-}" ]; then
  ODOO_IMAGE_TAG="$(tf_output_raw odoo_image_tag 2>/dev/null || echo "latest")"
fi
...
login_ecr() {
  if [[ "${DRY_RUN}" == "true" ]]; then
    echo "[odoo] (dry-run) aws --profile \"${AWS_PROFILE}\" ecr get-login-password --region \"${AWS_REGION}\" | docker login --username AWS --password-stdin \"${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com\""
    return 0
  fi
...
ensure_repo() {
  local repo="$1"
  if [[ "${DRY_RUN}" == "true" ]]; then
    echo "[odoo] (dry-run) ensure ECR repo exists: ${repo}"
...
ensure_local_image() {
  ...
  if [[ "${DRY_RUN}" == "true" ]]; then
    echo "[odoo] (dry-run) would require local image: ${img}"
    return 0
  fi
  if ! docker image inspect "${img}" >/dev/null 2>&1; then
    echo "[odoo] Local image ${img} not found. Run scripts/itsm/odoo/pull_odoo_image.sh first."
    exit 1
  fi
}
...
main() {
  ...
  echo "[odoo] AWS_PROFILE=${AWS_PROFILE} AWS_REGION=${AWS_REGION} AWS_ACCOUNT_ID=${AWS_ACCOUNT_ID}"
  echo "[odoo] ECR_URI=${ecr_uri}"
  if [[ "${DRY_RUN}" == "true" ]]; then
    echo "[odoo] DRY_RUN=true (no docker tag/push, no AWS calls)"
  fi
  ...
  if [[ "${DRY_RUN}" == "true" ]]; then
    echo "[odoo] (dry-run) docker tag \"${local_img}\" \"${ecr_uri}:latest\""
    echo "[odoo] (dry-run) docker push \"${ecr_uri}:latest\""
    echo "[odoo] (dry-run) would push ${ecr_uri}:latest"
  else
