...
# Redeploy GitLab Runner ECS service by forcing a new deployment.
...
if [ -z "${CLUSTER_NAME}" ] && [ "${DRY_RUN}" = "1" ]; then
  CLUSTER_NAME="<ECS_CLUSTER_NAME>"
fi
require_var "ECS_CLUSTER_NAME" "${CLUSTER_NAME}"

SERVICE_NAME="${SERVICE_NAME:-$(tf_output_raw gitlab_runner_service_name 2>/dev/null || true)}"
if [ -z "${SERVICE_NAME}" ] && [ "${DRY_RUN}" = "1" ]; then
