...
# Start the GitLab->EFS mirror Step Functions execution (looping).
#
...
STATE_MACHINE_ARN="$(tf_output_raw gitlab_efs_mirror_state_machine_arn || true)"
if [ -z "${STATE_MACHINE_ARN}" ]; then
  echo "[mirror] gitlab_efs_mirror_state_machine_arn is not available. Ensure:" >&2
  echo "  - enable_gitlab_efs_mirror=true" >&2
  echo "  - n8n EFS is configured" >&2
  echo "  - gitlab_admin_token is configured and written to SSM" >&2
...
if ! is_truthy "${FORCE_START}"; then
  ...
  if [ -n "${running}" ] && [ "${running}" != "None" ]; then
    echo "[mirror] already running: ${running}"
    echo "[mirror] set FORCE_START=true to start another execution (not recommended)."
    exit 0
  fi
fi

exec_name="gitlab-efs-mirror-$(date -u +%Y%m%dT%H%M%SZ)"

if is_truthy "${DRY_RUN}"; then
  echo "[mirror] DRY_RUN: would start execution:"
