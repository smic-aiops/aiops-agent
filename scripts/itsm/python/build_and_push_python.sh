...
# Load cached Python tarball from ./images and push to ECR.
#
# Usage:
#   scripts/itsm/python/build_and_push_python.sh [--dry-run]
#
...
#   ECR_REPO_PYTHON  (default: terraform output ecr_repo_python, fallback python)
...
usage() { echo "Usage: scripts/itsm/python/build_and_push_python.sh [--dry-run]"; }
...
if [ -z "${ECR_REPO_PYTHON:-}" ]; then ECR_REPO_PYTHON="$(tf_output_raw ecr_repo_python)"; fi
ECR_REPO_PYTHON="${ECR_REPO_PYTHON:-python}"
...
login_ecr() {
  if [[ "${DRY_RUN}" == "true" ]]; then
    echo "[python] (dry-run) docker login to ECR"
    return 0
  fi
...
ensure_repo() {
  local repo="$1"
  if [[ "${DRY_RUN}" == "true" ]]; then
    echo "[python] (dry-run) ensure ECR repo exists: ${repo}"
    return 0
  fi
  if ! aws --profile "${AWS_PROFILE}" ecr describe-repositories --repository-names "${repo}" --region "${AWS_REGION}" >/dev/null 2>&1; then
    ...
    echo "[python] Created ECR repo: ${repo}"
  fi
}

main() {
  local repo="${ECR_PREFIX}/${ECR_REPO_PYTHON}"
  local ecr_uri="${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${repo}"
  local tar_path="${IMAGES_DIR}/python/python-${PYTHON_TAG}.tar"
  local src="public.ecr.aws/docker/library/python:${PYTHON_TAG}"

  echo "[python] ECR_URI=${ecr_uri}"
  echo "[python] TAR=${tar_path}"

  login_ecr
  ensure_repo "${repo}"

  if [[ "${DRY_RUN}" == "true" ]]; then
    echo "[python] (dry-run) docker load -i \"${tar_path}\""
    echo "[python] (dry-run) docker tag \"public.ecr.aws/docker/library/python:${PYTHON_TAG}\" \"${ecr_uri}:${PYTHON_TAG}\""
    echo "[python] (dry-run) docker push \"${ecr_uri}:${PYTHON_TAG}\""
    echo "[python] (dry-run) docker tag \"${ecr_uri}:${PYTHON_TAG}\" \"${ecr_uri}:latest\""
    echo "[python] (dry-run) docker push \"${ecr_uri}:latest\""
    return 0
  fi

  docker_use_local_or_load_cache "python" "${src}" "${tar_path}" "run scripts/itsm/python/pull_python_image.sh"
  docker tag "${src}" "${ecr_uri}:${PYTHON_TAG}"
  docker push "${ecr_uri}:${PYTHON_TAG}"
  docker tag "${ecr_uri}:${PYTHON_TAG}" "${ecr_uri}:latest"
  docker push "${ecr_uri}:latest"
  echo "[python] Pushed: ${ecr_uri}:${PYTHON_TAG} and :latest"
}
