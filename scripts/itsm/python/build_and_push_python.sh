...
main() {
  ...
  if [[ ! -f "${tar_path}" ]]; then
    echo "[python] Missing cache tar: ${tar_path} (run scripts/itsm/python/pull_python_image.sh)" >&2
    exit 1
  fi
  ...
  docker tag "public.ecr.aws/docker/library/python:${PYTHON_TAG}" "${ecr_uri}:${PYTHON_TAG}"
