...
- **決定マーカー**: Zulip の `/decision`（既定）で始まる投稿は `apps/itsm_core/zulip_gitlab_issue_sync` が GitLab Issue に「決定（Zulip）」コメントとして証跡化する（マーカーは `ZULIP_GITLAB_DECISION_PREFIXES` で変更可）。GitLab 側で決定を記録する場合は `[DECISION]` / `決定:` を先頭に付け、Zulip へ通知する（詳細: `apps/itsm_core/zulip_gitlab_issue_sync/README.md`）。
...
- **GitLab 側の決定通知（補助）**: 例外的に GitLab 側で決定（`[DECISION]` / `決定:`）を記録した場合は、Zulip へ通知して関係者へ到達させる（詳細: `apps/itsm_core/zulip_gitlab_issue_sync/README.md`）。
